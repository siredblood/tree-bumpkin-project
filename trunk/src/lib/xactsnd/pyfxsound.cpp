...
PyFxSound::PyFxSound( const char* tag, PyTypePlus* pType ) :
	PyObjectPlus( pType ),
	//snd_( soundMgr().findFxSound(tag) ),	//TODO
	instance_( -1 ),
	position_( 0, 0, 0 )
{
}
...
static PyObject * py_playSimple( PyObject * args )
{
	...
	if (!PyArg_ParseTuple( args, "s", &tag ))
	{
		PyErr_SetString( PyExc_TypeError, "BigWorld.playSimple: Argument parsing error." );
		return NULL;
	}

	TRACE_MSG( "py_playSimple(%s)\n", tag );

	int event = SoundManager::instance().play( tag );
	PySound* sound = new PySound( event );

	return sound;
}
PY_MODULE_FUNCTION( playSimple, BigWorld )

static PyObject * py_getSimple( PyObject * args )
{
	char* tag;

	if (!PyArg_ParseTuple( args, "s", &tag ))
	{
		PyErr_SetString( PyExc_TypeError, "BigWorld.getSimple: Argument parsing error." );
		return NULL;
	}

	TRACE_MSG( "py_getSimple(%s)\n", tag );

	int event = SoundManager::instance().get( tag );
	PySound* sound = new PySound( event );

	return sound;
}
PY_MODULE_FUNCTION( getSimple, BigWorld )
