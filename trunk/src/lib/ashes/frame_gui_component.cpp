 ...
 *	@todo Comment
 */
void FrameGUIComponent::draw( bool overlay )
{
	if ( visible() )
	{
		Moo::rc().push();
		...
		for ( int i = 0; i < 4; i++ )
		{
			edges_[i]->draw( overlay );
			corners_[i]->draw( overlay );
		}
	}
 ...
 *	Load
 */
bool FrameGUIComponent::load( DataSectionPtr pSect, LoadBindings & bindings )
{
