				...
				const MethodDescription & description )
{
	...
	if (this != &description)
	{
		*static_cast<MemberDescription *>(this) = description;
	...
	if (PyTuple_Size( args ) != expectedArgs)
	{
		if (generateException)
		{
			PyErr_Format( PyExc_TypeError,
				"Method %s requires exactly %d argument%s; %zd given",
				name_.c_str(),
				expectedArgs,
				(expectedArgs == 1) ? "" : "s",
				PyTuple_Size( args ) );
			...
			if (peid == NULL || !PyInt_Check( peid ))
			{
				...
				if (generateException)
				{
					PyErr_Format( PyExc_TypeError,
						...
						"id as its first agument", name_.c_str() );
				}
				else
				{
					PyErr_Clear();
 ...
 *	@param pNub the nub to use, or NULL if none
 *
...
#if ENABLE_WATCHERS
	this->countReceived( data.remainingLength() );
#endif

	PyObjectPtr spTuple( this->getArgsAsTuple( data, sourceID,
		replyID, pReplyAddr, pNub ) );
...
#endif
		Script::call( pFunction, spTuple.getObject(),
 ...
 *	@param	pNub	pointer to the nub to use for replies, or NULL if none
	...
	Mercury::Nub* pNub) const
{
	...
	for (int i = 0; i < numArgs; i++)
	{
		...
		{
			Py_INCREF( &*pArg );
			PyTuple_SET_ITEM( pTuple, argAt++, &*pArg );
		}
