...
class EditorChunkUserDataObject : public EditorChunkSubstance<ChunkItem>, public Aligned
{
	...
	//change 20110726: 增加UserDataObject的临时存储
	static std::map<std::string, EditorChunkUserDataObject* > s_loadedObjects_;
	static void add( const std::string& guid, EditorChunkUserDataObject* );
...
public:
	...
	//change 20110726: 开放UserDataObject的获取接口
	static std::map<std::string, EditorChunkUserDataObject* > getUserDataObjects() { return s_loadedObjects_; }
	virtual std::string edGUID() { return guid_.toString(); }
