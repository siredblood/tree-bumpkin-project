...
bool BSP::save( FILE * pFile, const WorldTriangle * pFront ) const
{
	...
	{
		//ERROR_MSG( "BSP::save: There are too many triangles. %zu >= %d\n",
 ...
 *	in this subtree. This method expects numNodes to be initialized to some
...
bool BSPTree::save( const std::string & filename ) const
{
	...
	if (triangles_.size() > 0xffff)
	{
		...
		//		"Tree size (%zu) is bigger than max size (%d)\n",
