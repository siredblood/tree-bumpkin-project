 ...
 */
void TrackerNodeInfo::lookAt( Vector3& lookAtInWS, const Vector3 dirInWS,
 ...
 *	This method applies transforms so that nodes look at the specified vector.
 */
void ArmTrackerNodeInfo::lookAt( Vector3& lookAtInWS, const Vector3 dirInWS,
	const float weighting)
{
	BW_GUARD;
 ...
 *	This method applies transforms so that nodes look at the specified vector.
 */
void OrthogonalNodeInfo::lookAt( Vector3& lookAtInWS, const Vector3 dirInWS,
	const float weighting )
{
	BW_GUARD;
 ...
 *	This method applies transforms so that nodes look at the specified vector.
 */
void BlendNodeInfo::lookAt( Vector3& lookAtInWS, const Vector3 /*dirInWS*/,
	const float weighting)
{
	BW_GUARD;
...
int Tracker::pySet_directionProvider( PyObject * value )
{
	...
	{
		if (!pDirectionProvider_ && pCachedNodeInfo_)
		{
...
void Tracker::tick( float dTime, float lod )
{
	...
	if (pNodeInfo_)
	{
		...
		float modelPitch = atan2f( modelDir.y,
			sqrtf( modelDir.z * modelDir.z + modelDir.x * modelDir.x ) );
		float modelYaw = atan2f( modelDir.x, modelDir.z );
