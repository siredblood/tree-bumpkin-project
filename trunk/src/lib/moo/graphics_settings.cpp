 ...
 */
void GraphicsSetting::selectOption(int optionIndex,  bool force)
{
	BW_GUARD;
	// must be in this order, because needsRestart_ may change after
	// this->selectOptionPriv() gets called.
	if (this->selectOptionPriv(optionIndex, force) && this->needsRestart_)
	{
...
bool GraphicsSetting::selectOptionPriv(int optionIndex, bool force)
{
	...
	if (!force)
	{
		...
		{
			//ERROR_MSG("GraphicsSetting::selectOptionPriv - no supported option found for %s graphic setting", label_.c_str());		
		}
	}
