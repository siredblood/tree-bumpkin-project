 ...
 *  It is intended that EntityDescription and UserDataObjectDescription both descend from this class to minimize code 
 *  duplication.
 *
		...
		it != properties_.end(); ++it)
	{
		...
		if (pValue)
			PyDict_SetItemString( pDict,
				const_cast<char*>(pDesc->name().c_str()), &*pValue );
	}
}
