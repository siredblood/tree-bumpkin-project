	...
	if (pSpaceSection_)
	{
		...
		if (pBlock.hasObject())
		{
			...
			if (pCDataSection)
			{
				...
				Vector3 chunkPos( x * GRID_RESOLUTION, 0.0f, z * GRID_RESOLUTION );
				if (pTerrainBlock->load( spacePath_ + '/' + terrainSectionPath, chunkPos, chunkPos ))
				{
					Matrix chunkMtx( Matrix::identity );
					chunkMtx.translation( chunkPos );
					pTerrainBlock->rebuildCombinedLayers();
					pTerrainBlock->rebuildLodTexture( chunkMtx );
 ...
 */
void BlockConverter::init( DataSectionPtr pTerrainSection, TerrainSettingsPtr pSettings )
{
