 ...
 */
INLINE
void DataDescription::addToStream( PyObject * pNewValue,
			BinaryOStream & stream, bool isPersistentOnly ) const
{
	pDataType_->addToStream( pNewValue, stream, isPersistentOnly );
}

INLINE
void DataDescription::addToSection( PyObject * pNewValue,
			DataSectionPtr pSection )
{
	pDataType_->addToSection( pNewValue, pSection );
}
 ...
 */
INLINE
void DataDescription::fromStreamToSection( BinaryIStream & stream,
		DataSectionPtr pSection, bool isPersistentOnly ) const
{
	pDataType_->fromStreamToSection( stream, pSection, isPersistentOnly );
}
 ...
 */
INLINE
void DataDescription::fromSectionToStream( DataSectionPtr pSection,
			BinaryOStream & stream, bool isPersistentOnly ) const
{
	if (!pSection ||
		!pDataType_->fromSectionToStream( pSection, stream, isPersistentOnly ))
	{
		...
		pDataType_->addToStream( pValue.getObject(), stream, isPersistentOnly );
	}
}
...
bool DataDescription::isCellData() const
{
	...
	// still want one to be authorative.
...
/**
 * 	This method returns true if this property is client-only data. This
 * 	implies that the data is stored in the client scene graph, and is
 * 	entirely static. Therefore, the server never needs to know about it.
 */
INLINE
bool DataDescription::isClientOnlyData() const
{
	return !!(dataFlags_ & DATA_CLIENT_ONLY);
}
