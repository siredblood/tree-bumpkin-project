...
namespace
{
	...
	std::string getUniqueComputerName()
	{
		...
		GetComputerName( computerName, &nmLen );
...
bool WorldEditordConnection::connect()
{
	...
	{
		info << "WorldEditordConnection::Connect(): Couldn't resolve address " << host_ << "\n";
		//INFO_MSG( info.str().c_str() );
		addCommentary( L( "WORLDEDITOR/WORLDEDITOR/PROJECT/BIGBANGD_CONNECTION/CANNOT_RESOLVE_ADDR", host_ ), true );
		return false;
	}
	info.flush();
	...
	{
		info << "WorldEditordConnection::Connect(): Couldn't connect, last error is "<< WSAGetLastError() << "\n";
		//INFO_MSG( info.str().c_str() );
		addCommentary( L( "WORLDEDITOR/WORLDEDITOR/PROJECT/BIGBANGD_CONNECTION/CAMNOT_CONNECT", WSAGetLastError() ), true );
		return false;
	}

	connected_ = true;
	//INFO_MSG( "Connected to bwlockd\n" );
	addCommentary( L( "WORLDEDITOR/WORLDEDITOR/PROJECT/BIGBANGD_CONNECTION/CONNECTED" ), false );
...
void WorldEditordConnection::processInternalCommand( const std::vector<unsigned char>& comm )
{
	if( connected() )
	{
		...
		if( c->id_ == 'l' )
		{
			...
			addCommentary( L( "WORLDEDITOR/WORLDEDITOR/PROJECT/BIGBANGD_CONNECTION/LOCK",
				...
				if( stricmp( iter->name_.c_str(), computerName.c_str() ) == 0 )
				{
						...
						it != iter->locks_.end(); ++it )
					{
						if( it->rect_ == lock.rect_ )
						{
							...
							found = true;
							addCommentary( L( "WORLDEDITOR/WORLDEDITOR/PROJECT/BIGBANGD_CONNECTION/UNLOCK",
								lock.username_, computerName, lock.rect_.left_, lock.rect_.top_,
								lock.rect_.right_, lock.rect_.bottom_, lock.desc_ ), false );
							break;
						}
