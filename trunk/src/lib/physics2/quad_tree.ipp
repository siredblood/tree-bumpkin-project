	...
	headIndex_( 0 )
{
	...
	else
	{
		...
		xHigh0_ = -BIG_FLOAT;
		xHighDelta_ = BIG_FLOAT/highCrossing;
	}
	...
	else
	{
		...
		yHigh0_ = -BIG_FLOAT;
		yHighDelta_ = BIG_FLOAT/highCrossing;
	}
}
	...
	const QTRange & range, int depth )
{
	...
	{
		bool willGrow = elements_.size() == elements_.capacity();
		if (willGrow)
		{
			memoryCounterSub( quadTree );
			memoryClaim( elements_ );
		}
		elements_.push_back( &element );
		if (willGrow)
		{
			memoryCounterAdd( quadTree );
			memoryClaim( elements_ );
		}
	}
	else
