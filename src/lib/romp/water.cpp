...
bool Water::init()
{
	...
	if ( waterScene_ )
	{
		if ( Moo::EffectManager::instance().PSVersionCap()> 0 && !waterScene_->recreate() )
		{
...
{
	PROFILER_SCOPED( Water_setupRenderState );
	...
	{
		float w = (float)FullScreenBackBuffer::renderTarget().width();
		float h = (float)FullScreenBackBuffer::renderTarget().height();		
		float pixelX = 1.f / w;
		float pixelY = 1.f / h;
 ...
 *	The callback function takes one argument, a boolean describing whether
 *	the source location has just entered the water or not.
 *	@param	dynamicSource	A MatrixProvider providing the listener's location.
 *	@param	callback		A callback fn that takes one boolean argument.
 */
/**
