	...
	const BoundingBox & inbb )
{
	...
	*pTintPaletteConstant_ = s_particleTintPalette;
	...
	if (this->beginDraw(worldTransform, inbb))
	{
 ...
 */
// 修正看不见 [11/25/2008 chenqing]
//bool MeshParticleRenderer::beginDraw(const BoundingBox & inbb)
bool MeshParticleRenderer::beginDraw(const Matrix& worldTransform, const BoundingBox & inbb)
{
	BW_GUARD;
	Moo::rc().push();
	...
	// FIXED: <---
	BoundingBox bbox(inbb);
	if(local())
	{
		bbox.transformBy(worldTransform);
	}
	// --->

	if (s_helper.shouldDraw(false, bbox))
	{		
		s_helper.start( false, bbox );
...
void MeshParticleRenderer::draw( const BoundingBox& bb, int primGroupIndex )
{
	...
	else
	{
		...
		material_->channel()->addItem( pVSS, prim_, material_,
			primGroupIndex, distance, Moo::VertexBuffer() );
	}		
}
