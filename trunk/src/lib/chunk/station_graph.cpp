...
void StationGraph::registerNode( ChunkStationNode* node, Chunk* pChunk )
{
	...
	if (!isReady_)
	{
		...
		ChunkDirMapping* dirMapping = pChunk->mapping();
...
)
{
    ...
    graph1->loadAllChunks(dirMap);
    graph2->loadAllChunks(dirMap);
 ...
 *
 *  @param dirMap       The ChunkDirMapping used to load nodes with.
 */
void StationGraph::loadAllChunks(ChunkDirMapping *dirMap)
{
    BW_GUARD;
    ...
    )
    {
        ...
        std::string chunkName   = dirMap->outsideChunkIdentifier(localPos);
        if
        (
            !chunkName.empty()
            &&
...
/*virtual*/ bool StationGraph::isValid( std::string &failureMsg ) const
{
    ...
    {
        failureMsg = L("CHUNK/INHABITANTS/STATION_GRAPH/NODE_SIZE_UNEQUAL");
        return false;
    }
