	...
	const BoundingBox &bb )
{
	...
	if (beg != end)
	{
		...
		Moo::SortedChannel::addDrawItem( new ParticleSystemDrawItem( this, worldTransform, beg, end, distance ) );		
	}
}
...
void SpriteParticleRenderer::realDraw( const Matrix& worldTransform, Particles::iterator beg, Particles::iterator end )
{
	...
	uint32 state = Moo::rc().getRenderState( D3DRS_COLORWRITEENABLE );
	...
	while (it != end)
	{
		...
		float screenSpaceSize = size / position.z;
		if (screenSpaceSize < minSize)
			continue;
		...
		rotationSpeed *= MATH_2PI;	// pitch is in rev / s
		...
		{		
			...
			xdir = view.applyVector(xdir*size);
			ydir = view.applyVector(ydir*size);
		...
		if ( materialFX() == FX_ADDITIVE_ALPHA )
		{
	...
	{
		vb.set( 0 );
		Moo::rc().drawPrimitive( D3DPT_TRIANGLELIST, lockIndex, numTris );
	}

	// set color write enable in case we are shimmering
	Moo::rc().setRenderState( D3DRS_COLORWRITEENABLE, state );
}
