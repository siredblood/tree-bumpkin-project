...
struct TabCtrlItem
{
	...
	TabCtrlItem( const std::string& c, HICON i, void* d ) :
		caption( c ),
		icon( i ),
		itemData( d ),
		curLeft( 0 ),
...
public:
	...
	void insertItem( const std::string& caption, HICON icon, void* itemData );
	...
	void updateItemData( void* itemData, const std::string& caption, HICON icon );
