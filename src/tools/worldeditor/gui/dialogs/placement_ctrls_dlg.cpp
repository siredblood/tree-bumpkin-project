...
const std::string PlacementCtrlsDlg::contentID = "PlacementCtrlsDlg";
...
void PlacementCtrlsDlg::loadPreset()
{
	...
	{
		name.Format( "min%s%c",
			sliders_[j]->group_ == GROUP_ROTATION ? "Rot" : "Sca",
			axis[ j % 3 ] );
		float minVal = preset->readFloat( (LPCTSTR)name );

		name.Format( "max%s%c",
			sliders_[j]->group_ == GROUP_ROTATION ? "Rot" : "Sca",
			axis[ j % 3 ] );
		float maxVal = preset->readFloat( (LPCTSTR)name );
...
void PlacementCtrlsDlg::savePreset()
{
	...
	for( int j = 0; j < (int)sliders_.size(); ++j )
	{
		name.Format( "min%s%c",
			sliders_[j]->group_ == GROUP_ROTATION ? "Rot" : "Sca",
			axis[ j % 3 ] );
		preset->writeFloat( (LPCTSTR)name, sliders_[j]->minEd_->GetValue() );

		name.Format( "max%s%c",
			sliders_[j]->group_ == GROUP_ROTATION ? "Rot" : "Sca",
			axis[ j % 3 ] );
		preset->writeFloat( (LPCTSTR)name, sliders_[j]->maxEd_->GetValue() );
	}
...
bool PlacementCtrlsDlg::newPreset( GUI::ItemPtr item )
{
	...
	preset->writeString( "displayName", (LPCTSTR)newPlacementDlg_.newName_ );
...
{
	if ( MessageBox( L("WORLDEDITOR/GUI/PLACEMENT_CTRLS_DLG/DELETE_PRESET_TEXT"),
		L("WORLDEDITOR/GUI/PLACEMENT_CTRLS_DLG/DELETE_PRESET_TITLE"),
