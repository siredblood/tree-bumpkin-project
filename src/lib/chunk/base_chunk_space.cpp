...
Chunk * BaseChunkSpace::Column::findChunk( const Vector3 & point )
{
	...
	if (pChunkTree_ != NULL)
	{
		...
		while (result != NULL)
		{
			if ( ( pChunk == pOutsideChunk_ || pChunk->volume() > result->pChunk_->volume() )
				&& result->pChunk_->boundingBox().intersects( point ) )
	...
	const std::string & mappingName )
{
	...
	if (found == currentChunks_.end()) return NULL;
	for (uint i = 0; i < found->second.size(); i++)
	{
	...
	const SpaceEntryID & entryID, uint16 key )
{
	...
	if (key == uint16(-1))
	{
		...
		if (it != dataEntries_.end())
			...
			return DataValueReturn( uint16(-1), NULL );
	}
	else
	{
		DataEntryMapKey demk;
 ...
 */
void BaseChunkSpace::emulate( SmartPointer<BaseChunkSpace> pRightfulSpace )
{
	BW_GUARD;
	id_ = pRightfulSpace->id_;
	dataEntries_ = pRightfulSpace->dataEntries_;
	MF_ASSERT_DEV( pRightfulSpace->currentChunks_.empty() );
}
