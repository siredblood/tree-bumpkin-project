...
ForwardingCollector::~ForwardingCollector()
{
	...
	// We don't own the nub, so don't destroy it
	pNub_ = NULL;
}
 ...
 * @param nub The Mercury Nub to use to send messages from.
...
bool ForwardingCollector::init( const Mercury::InterfaceElement & ie,
	Mercury::Nub & nub, AddressList *addrList )
{
	...
	BinaryIStream & data, void * arg)
{
	...
	{
		ERROR_MSG( "ForwardingCollector::handleMessage: Received a response "
					"from an unknown address (%s).\n", (char *)addr );
		return;
	}
