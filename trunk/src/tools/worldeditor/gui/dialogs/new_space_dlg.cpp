...
BOOL NewSpaceDlg::OnInitDialog()
{
	...
	{
		bw_snprintf(buffer, sizeof(buffer), "%d", size );
	...
	itoa( Options::getOptionInt( "terrain2/defaults/heightMapSize", 
		Terrain::TerrainSettings::defaults()->heightMapSize() ), buffer, 10 );
	heightMapSize_.SelectString( -1, buffer );
	itoa( Options::getOptionInt( "terrain2/defaults/normalMapSize", 
	...
	itoa( Options::getOptionInt( "terrain2/defaults/shadowMapSize", 
	...
	defaultTexture_.SetWindowText( Options::getOptionString( LAST_DEFAULT_TEXTURE_TAG ).c_str() );
	...
	bool createTemplateTerrainBlock()
	{
		...
		else
		{
			...
			else
			{
				...
				block->rebuildLodTexture( Matrix::identity );
			}			
		...
		if ( !defaultTexture_.empty() )
		{
			...
			if ( createLegacy_ )
				CopyFile( templateTname.c_str(), fname.c_str(), FALSE ); // need a temp file for old spaces
			...
			if ( block )
			{
				...
				if ( block->rebuildLodTexture( m ) )
				{
					...
					if ( cdataDS )
					{
						...
						if ( terrainDS && block->save( terrainDS ) )
							cdataDS->save();
						//else
...
void NewSpaceDlg::OnBnClickedOk()
{
	...
	ny.Format( "%d", MIN_CHUNK_HEIGHT );
	my.Format( "%d", MAX_CHUNK_HEIGHT );
	...
	else if ( BWResource::fileExists( spaceName + "/" + SPACE_SETTING_FILE_NAME ) )
	{
	...
	else
	{
		...
		if ( !textureImage_.image().isEmpty() )
		{
			...
			std::string dissolvedFileName = BWResource::dissolveFilename( dt.GetString() );
...
bool NewSpaceDlg::validateDefaultTexture()
{
	if ( defaultTexture_.GetWindowTextLength() > 0 && textureImage_.image().isEmpty() )
	{
		CString dt;
		defaultTexture_.GetWindowText( dt );
		std::string resolvedFName = BWResource::resolveFilename( dt.GetString() );
		std::string msg = "The Default Terrain Texture '" + resolvedFName +
			"' cannot be used because it's missing or it's not in a valid resource path.\n" +
			"To create the space, please enter a valid texture file name, or clear the Default Terrain Texture field.";
		MessageBox( msg.c_str(), "Invalid Default Terrain Texture", MB_ICONERROR );
		return false;
	}
...
void NewSpaceDlg::OnEnTextureChange()
{
	...
	textureImage_.image().load(
		BWResource::resolveFilename( dt.GetString() ) );
...
void NewSpaceDlg::OnBnTextureBrowse()
{
	static char szFilter[] = 
        "Texture Files (*.tga;*.bmp;*.dds)|*.tga;*.bmp;*.dds|"
        "All Files (*.*)|*.*||";
	...
	std::string lastDefTex;
	if ( !dt.IsEmpty() )
	{
		lastDefTex = 
			BWResource::getFilePath(
				BWResource::resolveFilename( dt.GetString() ) );
		std::replace( lastDefTex.begin(), lastDefTex.end(), '/', '\\' );
	...
	{
		bool failed = true;
		std::string dissolvedFileName = BWResource::dissolveFilename( fileDialog.GetPathName().GetString() );
		if ( dissolvedFileName[1] != ':' && dissolvedFileName.size() )
		{
			defaultTexture_.SetWindowText( dissolvedFileName.c_str() );
			OnEnTextureChange();
			failed = false;
		}
		...
		if ( textureImage_.image().isEmpty() || failed )
		{
			...
			MessageBox( msg.c_str(), "Browse for Default Terrain Texture", MB_ICONERROR );
		}
	}
}
...
void NewSpaceDlg::OnBnClickedLegacy()
{
	...
	if ( enable )
	{
		...
		heightMapSize_.DeleteString( heightMapSize_.FindStringExact( -1, "25" ) );
		itoa( Options::getOptionInt( "terrain2/defaults/heightMapSize", 
			Terrain::TerrainSettings::defaults()->heightMapSize() ), buffer, 10 );
		heightMapSize_.SelectString( 0, buffer );
		normalMapSize_.DeleteString( normalMapSize_.FindStringExact( -1, "25" ) );
		itoa( Options::getOptionInt( "terrain2/defaults/normalMapSize", 
			Terrain::TerrainSettings::defaults()->normalMapSize() ), buffer, 10 );
		...
		holeMapSize_.SetIntegerValue(
			Options::getOptionInt( "terrain2/defaults/holeMapSize", 
			Terrain::TerrainSettings::defaults()->holeMapSize() ) );
		shadowMapSize_.DeleteString( shadowMapSize_.FindStringExact( -1, "25" ) );
		itoa( Options::getOptionInt( "terrain2/defaults/shadowMapSize", 
			Terrain::TerrainSettings::defaults()->shadowMapSize() ), buffer, 10 );
	...
	else
	{
		...
		heightMapSize_.InsertString( 0, "25" );
		heightMapSize_.SelectString( -1, "25" );
		normalMapSize_.InsertString( 0, "25" );
		normalMapSize_.SelectString( -1, "25" );
		holeMapSize_.SetIntegerValue( 25 );
		shadowMapSize_.InsertString( 0, "25" );
		shadowMapSize_.SelectString( -1, "25" );
