...
{
	const Terrain::EditorBaseTerrainBlock * pBlock = &terrain->block();
	maskLayers.resize( pBlock->numberTextureLayers() );
	if (brush.textureMask_)
	{				
		for (size_t i = 0; i < pBlock->numberTextureLayers(); ++i)
		{
			Terrain::TerrainTextureLayer &layer = 
				const_cast< Terrain::TerrainTextureLayer & >( pBlock->textureLayer( i ) );
			layer.lock( true );
...
)
{
	...
	{
		float h = pBlock->heightMap().heightAt( pos.x, pos.z );
	...
	if (brush.slopeMask_)
	{
		float angle = pBlock->heightMap().slopeAt( pos.x, pos.z );
 ...
 */
MaskResult * TextureMaskCache::mask( EditorChunkTerrainPtr terrain )
{
 ...
 *					to be recalculated.
 */
void TextureMaskCache::changedLayers( EditorChunkTerrainPtr terrain )
{
	CacheMap::iterator it = cache_.find( terrain );
	if (it != cache_.end())
	{
		MaskResult &result = it->second;
		const Terrain::EditorBaseTerrainBlock * pBlock = &it->first->block();
 ...
 *	@return 		A pointer to the mask image for the terrain.
 */
MaskResult * TextureMaskCache::generate( EditorChunkTerrainPtr terrain )
{
	if (terrain == NULL)
	{
		return NULL;
	...
	const Terrain::EditorBaseTerrainBlock * pBlock = &terrain->block();
	if (pBlock->numberTextureLayers() == 0)
	{
		return NULL;
	}

	Vector3 offset = terrain->chunk()->transform().applyToOrigin();

	// Get the size of the mask:
	Terrain::TerrainTextureLayer const &ttl = pBlock->textureLayer( 0 );
