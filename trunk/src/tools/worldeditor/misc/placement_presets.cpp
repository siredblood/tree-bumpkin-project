...
void PlacementPresets::readPresets()
{
		...
		if ( (*i) != dialogCombo )
		{
			item = new GUI::Item( "CHILD",
				...
				section_->readString( "editPresetsString",
					L("WORLDEDITOR/WORLDEDITOR/TOOL/PLACEMENT_PRESETS/EDIT_PRESET") ),
		...
		c != comboBoxes_.end(); ++c )
	{
		...
		// add default non-random placement item
		int idx = (*c)->AddString( section_->readString( "editPresetsString",
			L("WORLDEDITOR/WORLDEDITOR/TOOL/PLACEMENT_PRESETS/EDIT_PRESET") ).c_str() );
		(*c)->SetItemData( idx, PLACECTRL_EDIT_INT );
		idx = (*c)->AddString( section_->readString( "normalPlacementString",
			L("WORLDEDITOR/WORLDEDITOR/TOOL/PLACEMENT_PRESETS/EDIT_PRESET") ).c_str() );
		...
		for( int i = 0; i < presetsSection_->countChildren(); ++i )
		{
			...
			if ( itemData )
			{
				idx = (*c)->AddString( itemData->readString( "displayName", "" ).c_str() );
				(*c)->SetItemData( idx, i );
			}
		}
	}
...
void PlacementPresets::presetNames(std::vector<std::string> &names) const
{
	for( int i = 0; i < presetsSection_->countChildren(); ++i )
	{
		DataSectionPtr preset = presetsSection_->openChild( i );
		std::string name = preset->readString( "displayName" );
		if (!name.empty())
		{
