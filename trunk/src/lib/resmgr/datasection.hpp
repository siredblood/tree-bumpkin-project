...
namespace Datatype {
	...
	// BC: added specializations for built-in
	// types to avoid uninitialized memory reads
...
template <class C> struct DataSectionTypeConverter
{
	...
	static bool be( DataSectionPtr pSection, const C & value,
			const char * format = "" )
		{ return C::DataSection_be( pSection, value, format ); }
};
...
public:
	...
	template <class C> bool be( const C & value, const char * format )
		{ return DataSectionTypeConverter<C>::be( this, value, format ); }

	/// This method sets the datasection to a boolean value.
	virtual bool setBool( bool value, const char *format = "%s" );

	/// This method sets the datasection to an integer value.
	virtual bool setInt( int value, const char *format = "%d" );

	/// This method sets the datasection to a long value.
	virtual bool setLong( long value, const char *format = "%d" );

	/// This method sets the datasection to a floating-point value.
	virtual bool setFloat( float value, const char *format = "%f" );

	/// This method sets the datasection to a double value.
	virtual bool setDouble( double value, const char *format = "%f" );
	...
	virtual bool setVector2( const Vector2 &value, const char *format = "%f %f" );

	/// This method sets the datasection to a Vector3 value.
	virtual bool setVector3( const Vector3 &value, const char *format = "%f %f %f" );

	/// This method sets the datasection to a Vector4 value.
	virtual bool setVector4( const Vector4 &value, const char *format = "%f %f %f %f" );

	/// This method sets the datasection to a Matrix34 value.
	virtual bool setMatrix34( const Matrix &value,
		const char *vectorFormat = "%f %f %f" );
		...
		const char * format )
	{
		...
		return pSection->be( value, format );
	}
	...
	bool writeBool(
			const std::string &tagPath, bool value, const char *format = "%s" );

	/// This method writes an integer value to the tag specified.
	bool writeInt( const std::string &tagPath, int value, const char *format = "%d" );

	/// This method writes a long value to the tag specified.
	bool writeLong(
			const std::string &tagPath, long value, const char *format = "%d" );

	/// This method writes a floating-point value to the tag specified.
	bool writeFloat(
			const std::string &tagPath, float value, const char *format = "%f" );

	/// This method writes a double floating-point value to the tag specified.
	bool writeDouble(
			const std::string &tagPath, double value, const char *format = "%f" );
	...
	bool writeBinary( const std::string& tagPath,
			BinaryPtr pBinary );
	...
	template <class SEQ> void writeSeq( const std::string & tagPath, SEQ & src,
		bool replaceExisting, const char * format )
	{
		std::string tag;
		DataSectionPtr pDest = this->splitTagPath( tagPath, tag, false );
		if (!pDest) return;
		...
		for (typename SEQ::iterator it = src.begin(); it != src.end(); it++)
		{
			pDest->newSection( tag )->be( *it, format );
		}
	}
...
#define BUILTIN_TEMPLATES( TYPE, AS_FUNCTION, BE_FUNCTION )					\
	...
	{																		\
		...
		static bool be( DataSectionPtr pSection, const TYPE & value,		\
				const char * f )											\
			{ return pSection->BE_FUNCTION( value, const_cast<char*>(f) ); }\
	};

#define BUILTIN_TEMPLATES_NOFORMAT( TYPE, AS_FUNCTION, BE_FUNCTION )		\
	/**																		\
	 * TODO: to be documented.												\
	 */																		\
	template <> struct DataSectionTypeConverter<TYPE>						\
...
BUILTIN_TEMPLATES_NOFORMAT( std::string, asString, setString )
BUILTIN_TEMPLATES_NOFORMAT( std::wstring, asWideString, setWideString )
...
#define BUILTIN_TEMPLATES_PROMOTE( TYPE, DSTYPE, AS_FUNCTION, BE_FUNCTION )	\
	...
	{																		\
		static TYPE as( DataSectionPtr pSection, const TYPE& defaultVal )	\
			{ return TYPE( pSection->AS_FUNCTION( DSTYPE(defaultVal) ) ); }	\
		static TYPE as( DataSectionPtr pSection )							\
			{ return TYPE( pSection->AS_FUNCTION() ); }						\
		static bool be( DataSectionPtr pSection, const TYPE& value )		\
			{ return pSection->BE_FUNCTION( DSTYPE(value) ); }				\
		static bool be( DataSectionPtr pSection, const TYPE & value,		\
				const char * f )											\
			{ return pSection->BE_FUNCTION( DSTYPE(value),					\
											const_cast<char*>(f) ); }		\
...
// __kyl__ (1/8/2005) The following causes a bit of weirdness i.e. you need
// to specify large positive numbers using negative numbers in your XML
// file.
BUILTIN_TEMPLATES_PROMOTE( uint32, int, asInt, setInt )

// specialisations for signed/unsigned integers of different sizes by using
// string functions.
#define BUILTIN_TEMPLATES_USESTRING( TYPE )									\
	template <> struct DataSectionTypeConverter<TYPE>						\
	{																		\
		static TYPE as( DataSectionPtr pSection,							\
						const TYPE& defaultVal =  TYPE() )					\
			{																\
				std::istringstream in( pSection->asString() );				\
				TYPE val;													\
				in >> val;													\
				if (in.fail())												\
					val = defaultVal;										\
		...
		static bool be( DataSectionPtr pSection, const TYPE& value )		\
			{ 																\
				...
				out << value;												\
				return pSection->setString( out.str() );					\
			}																\
	};

#ifndef _LP64
BUILTIN_TEMPLATES_USESTRING( int64 )
BUILTIN_TEMPLATES_USESTRING( uint64 )
#else
BUILTIN_TEMPLATES_PROMOTE( uint64, long, asLong, setLong )
#endif
