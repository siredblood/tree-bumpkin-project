...
private:
	...
	struct AQElement
	{
		...
		AQElement( const AQElement & other ) :
			action( other.action ),
			responses( NULL ),
			promoteMotion( other.promoteMotion ),
			link( NULL )
		{
			if (other.responses != NULL)
				responses = new CueResponses( *other.responses );
			if (other.link != NULL)
				link = new AQElement( *other.link );
		}
		AQElement & operator=( const AQElement & other )
		{
			if (&other != this)
			{
				CueResponses * oresponses = responses;
