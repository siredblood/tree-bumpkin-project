...
public:
	...
	PyObject * get( const std::string & key, PyObject * defVal );
	PY_AUTO_METHOD_DECLARE( RETOWN, get, ARG( std::string,
		OPTARG( PyObject *, NULL, END ) ) )
 ...
 */
PyObject * ResTblStruct::get( const std::string & key, PyObject * defVal )
{
	...
	if (defVal == NULL) { Py_Return; }	// convert NULL back to None
	//PY_XINCREF( defVal ); // we don't need to incref it since we use the
	return defVal;			// reference passed in from Script::getData
}
...
PyMappingMethods ResourceTable::s_map_methods_ =
{
	...
	index_( ~0u )
{
	...
	ResTblStructPtr pParentStruct = pParent_ ?
		(ResTblStruct*)&*pParent_->pValue_ : NULL;
...
PyObject * ResourceTable::pyGetAttribute( const char * attr )
{
	...
	PyObject * tryRet = PyObject_GetAttrString( &*pValue_, (char*)attr );
 ...
 */
int ResourceTable::pyMap_length()
{
...
PyObject * ResourceTable::at( int index )
{
	...
	PyObject * pRet = &*keyedEntries_[ widx ];
	Py_INCREF( pRet );
	return pRet;
}
 ...
 */
struct RTSCompare { bool operator()(
	const ResourceTable * a, const ResourceTable * b ) const {
		return &*a->pSect_ < &*b->pSect_; } };
	...
	PyObjectPtr updateFn )
{
	...
	if (updateFn && !PyCallable_Check( &*updateFn ))
	{
		PyErr_SetString( PyExc_TypeError,
