...
public:
	...
	BwFont::FontPtr font()					{ return font_; }
	int visibleWidth() const;
	int visibleHeight() const;

	const uint8 * buffer() const	{ return &buffer_[0][0]; }
	uint8 * buffer()				{ return &buffer_[0][0]; }
	//@}
...
protected:
	...
	typedef std::vector<BwFont::FontPtr> FontVector;
	FontVector fonts_;

	// console buffer
	uint8			buffer_[MAX_CONSOLE_HEIGHT][MAX_CONSOLE_WIDTH];
