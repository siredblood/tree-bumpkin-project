...
std::string StringUtils::matrixToString( const Matrix& transform, char separator /* = ';' */ )
{
	std::string ret;
	for ( int n = 0; n < 4; n++ )
	{
		...
		CString strPos;
		strPos.Format( "%.4f,%.4f,%.4f", transform[ n ].x, transform[ n ].y, transform[ n ].z );
		
		ret += strPos.GetBuffer( 0 );
...
void StringUtils::matrixFromString( const std::string& str, Matrix& transform, char* separator /* = ";" */ )
{
	char* resToken;
	int curPos = 0;

	resToken = strtok( (char*)str.c_str(), separator );
	while( resToken != 0 )
	{
		Vector3 v;
		sscanf( resToken, "%f,%f,%f", &v.x, &v.y, &v.z );	
		transform[ curPos ] = v;
		
		curPos ++;
		resToken = strtok( 0, separator );	
	}
}
