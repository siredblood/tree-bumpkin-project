 ...
 */
bool SpeedTreeThumbProv::isValid(const ThumbnailManager& manager, const std::string& file)
{
	if (file.empty())
		return false;
 ...
 */
bool SpeedTreeThumbProv::prepare(const ThumbnailManager& manager, const std::string& file)
{
	bool result = false;
	try
	{
		...
		std::string resFile = BWResource::dissolveFilename(file);
 ...
 */
bool SpeedTreeThumbProv::render(const ThumbnailManager& manager, const std::string& file, Moo::RenderTarget* rt)
{
	if ( !tree_ )
		return false;
