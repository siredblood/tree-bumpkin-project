...
PyModel::PyModel( SuperModel * pSuperModel, PyTypePlus * pType ) :
 ...
 boundingBox_( Vector3::zero(), Vector3::zero() ),
 visibilityBox_( Vector3::zero(), Vector3::zero() ),
 height_( 0.f ),
 visible_( true ),
...
void PyModel::calculateBoundingBoxes()
{
	...
	if (pSuperModel_ != NULL)
	{
		...
		visibilityBox_ = BoundingBox(
			Vector3( -maxXZ, bb.minBounds().y - ysz*0.5f, -maxXZ ),
			Vector3( maxXZ, bb.maxBounds().y + ysz*0.5f, maxXZ ) );
	}
	else
	{
		boundingBox_.setBounds( Vector3(-0.5f,  0.0f, -0.5f),
								Vector3( 0.5f,  1.0f,  0.5f) );
		visibilityBox_ = boundingBox_;
	}
}
 ...
 */
PyObject * PyModel::pyGet_yaw()
{
	BW_GUARD;
	const Matrix & wM = this->worldTransform();
	float	yaw = atan2f( wM.m[2][0], wM.m[2][2] );
	return Script::getData( yaw );
}

/**
 *	Specialised set method for the 'yaw' attribute
 */
int PyModel::pySet_yaw( PyObject * value )
{
	BW_GUARD;
	Matrix	wM = this->worldTransform();
	float	yaw;
	int ret = Script::setData( value, yaw, "yaw" );
	if (ret == 0)
	{
 ...
 *	the moment.
 */
int PyModel::pySet_height( PyObject * value )
{
	BW_GUARD;
	int err = Script::setData( value, height_, "PyModel.height" );
	if (err) return err;
...
public:
	...
	virtual void matrix( Matrix & m ) const
	{
		...
		pModel_->boundingBoxAcc( bb, true );
...
PyObject * PyModel::node( const std::string & nodeName )
{
	...
	PyModelNode * pPyNode = this->accessNode( pNode );
	Py_INCREF( pPyNode );
	return pPyNode;
}
 ...
 */
PyModelNode * PyModel::accessNode( Moo::NodePtr pNode )
{
	BW_GUARD;
	PyModelNodes::iterator nit;
	for (nit = knownNodes_.begin(); nit != knownNodes_.end(); nit++)
	{
		if ((*nit)->pNode() == pNode) break;
	}
	if (nit == knownNodes_.end())
	{
		knownNodes_.push_back( new PyModelNode( this, pNode ) );
 ...
 *
 *  For sound naming semantics, please see BigWorld.playSound().
 *
...
PyObject* PyModel::getSound( const std::string& s )
{
	...
	SoundManager::Event *pEvent = SoundManager::instance().get( s );

	if (pEvent == NULL)
		return SoundManager::error();

	if (!this->attachSound( pEvent ))
		return SoundManager::error();

	PySound *pSound = new PySound( pEvent, s );
	pSound->setModel( this );
	return pSound;
}
...
{
	BW_GUARD;
	PyObject *pSound = this->getSound( s );

#if FMOD_SUPPORT
	if (pSound != SoundManager::error())
	{
		((PySound*)pSound)->pEvent()->start();
	}
#endif
 ...
 *  necessary.
 */
SoundManager::EventList& PyModel::sounds()
{
	BW_GUARD;
	if (pSounds_ == NULL)
		pSounds_ = new SoundManager::EventList();
 ...
 */
bool PyModel::attachSound( SoundManager::Event *pEvent )
{
	BW_GUARD;
	if (SoundManager::instance().set3D(
			pEvent,	this->worldTransform().applyToOrigin()))
	{
		this->sounds().push_back( pEvent );
		return true;
	}
	else
		return false;
 ...
 */
void PyModel::visibilityBoxAcc( BoundingBox & vbb )
{
	BW_GUARD;
	if ((!this->visible()) && (!this->visibleAttachments())) return;

	vbb.addBounds( visibilityBox_ );

	// TODO: Don't do all this every frame! Store a maximum BB for most
	// things, and for special dynamic ones (particle systems) ask them
	// every few seconds or some such...
	if (knownNodes_.empty()) return;

	// tbb is in world coords
	BoundingBox tbb = BoundingBox::s_insideOut_;

	PyModelNodes::iterator nit;
	for (nit = knownNodes_.begin(); nit != knownNodes_.end(); nit++)
	{
		(*nit)->boundingBoxAcc( tbb );
	}

	if( tbb.insideOut() )
		return;

	// put into model-local coords
	if (pOwnWorld_ != NULL)
	{
		Matrix fastInvWorld;
		const Matrix & world = pOwnWorld_->getMatrix();
		const Vector3 & worldPos = world.applyToOrigin();
		fastInvWorld.setRotateY( -world.yaw() );
		tbb.setBounds(	tbb.minBounds() - worldPos,
						tbb.maxBounds() - worldPos );
 ...
 */
void PyModel::boundingBoxAcc( BoundingBox & bb, bool skinny )
{
	BW_GUARD;
	if (skinny)
	{
...
void PyModel::drawDebugInfo()
{
	...
	static BoundingBoxGUIComponent	*bbgc = NULL;
	static TextGUIComponent			*ditext = NULL;
	...
	if (!initted)
	{
		...
		bbgc = new BoundingBoxGUIComponent("");
		bbgc->width( 16 );
		bbgc->height( 16 );
		bbgc->colour( 0 );

		BwFont::FontPtr xf = BwFont::FontManager::instance().get( "system_small.font" );

		ditext = new TextGUIComponent( xf );
		ditext->colour( Colour::getUint32( 64, 255, 0, 0 ) );
		ditext->anchor( SimpleGUIComponent::ANCHOR_H_CENTER,
			SimpleGUIComponent::ANCHOR_V_TOP );

		/* set the material up properly */
		ditext->materialFX( SimpleGUIComponent::FX_BLEND );

		bbgc->addChild( "dt", ditext );
	}

	MatrixProviderPtr pMProv( new ModelBBMProv( this ), true );
	bbgc->source( pMProv );
	...
	for (DebugStrings::iterator i = pDebugInfo_->begin();
		...
		i++)
	{
		ditext->position( Vector3( 0.5, hup, 1 ) );
		ditext->slimLabel( std::string(i->first) + ": " + i->second );

		bbgc->update( 0, Moo::rc().screenWidth(), Moo::rc().screenHeight() );
//		bbgc->material().setZBufferWrite( false );
		bbgc->draw();
...
static SmartPointer<PyModel>	s_dpm;
...
void PyModel::debugStuff( float dTime )
{
	...
	if (!s_dpm || !s_dpm->isInWorld()) return;
	s_dpm->actionQueue().debugTick( dTime );
	s_dpm->actionQueue().debugDraw();
#endif
}
...
static void debugAQ( SmartPointer<PyModel> pm )
{
	BW_GUARD;	
#if ENABLE_ACTION_QUEUE_DEBUGGER
	if (pm) pm->actionQueue().debugTick( -1.f );
	s_dpm = pm;
#else
