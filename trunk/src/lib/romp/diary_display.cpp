...
CustomMesh<Moo::VertexXYZDUV> DiaryDisplay::textMesh_(D3DPT_TRIANGLELIST);
...
void DiaryDisplay::displayAll()
{
	...
	for (uint i = 0; i < disz; i++)
	{
		...
		DiaryDisplay::calculateDisplay( entries, height, xAxis, curEnd );		
		height -= heightEach;
	}

	DiaryDisplay::drawAxis(xAxis, true, shouldDisplayXAxisTimes);
	DiaryDisplay::drawDisplay();
...
void DiaryDisplay::drawDisplay()
{
	...
	if (!textMesh_.empty())
	{
		static BwFont::FontPtr myfont = BwFont::FontManager::instance().get( "default_small.font" );
...
{
	static BwFont::FontPtr myfont = BwFont::FontManager::instance().get( "system_small.font" );	
	CustomMesh<Moo::VertexXYZDUV> textMesh_;
...
void DiaryDisplay::calculateDisplay( const DiaryEntries & entries,
	float height, XAxis& xAxis, uint64 now, int64 syncOffset )
{	
	...
	float fHeight = myfont->metrics().clipHeight() * Moo::rc().halfScreenHeight();
	...
	for (int i = int(entries.size())-1; i >= 0; i--)
	{
		...
		if (nRows > 0 && width > 1)
		{
			...
			int sWidth = myfont->metrics().stringWidth( t );
			std::string label;

			if (width > sWidth)
			{
				//Enough room for at least the type
				sWidth = myfont->metrics().stringWidth( de.desc_ );
				if (width > sWidth)
				{
			...
			{							
				...
				sWidth = myfont->metrics().stringWidth(t.substr(0,1));
				if (width > sWidth )
				{
			...
			if ( !label.empty() )
			{
				...
				myfont->colour( Colour::getUint32FromNormalised( (Vector4&)col ) );
				myfont->drawIntoMesh( textMesh_, buf, clipPos.x, clipPos.y );
			}
			...
			if (nRows > 1)
			{
				...
				int sWidth = myfont->metrics().stringWidth( ss.str() );
				if (width > sWidth)
				{
					wchar_t buf[256];					
					bw_snwprintf( buf, sizeof(buf)/sizeof(wchar_t), L"%S\0", ss.str() );
					clipPos.y -= myfont->metrics().clipHeight();
					myfont->drawIntoMesh( textMesh_, buf, clipPos.x, clipPos.y );
				}
			}
		}		
...
{
	font_ = BwFont::FontManager::instance().get( "system_small.font" );
}
...
{	
	...
	int fHeight = (int)font_->metrics().height();
