...
//change 20110726:自定义接口 添加
std::map<std::string, EditorChunkUserDataObject* > EditorChunkUserDataObject::s_loadedObjects_;
void EditorChunkUserDataObject::add( const std::string& guid, EditorChunkUserDataObject* object_ )
{
	std::map< std::string, EditorChunkUserDataObject* >::iterator iter = 
		s_loadedObjects_.find( guid );
		
	if ( iter == s_loadedObjects_.end() )
		s_loadedObjects_.insert( std::make_pair( guid, object_ ) );	
}

//change 20110726:自定义接口 移除
void EditorChunkUserDataObject::remove( const std::string& guid )
{
	std::map<std::string, EditorChunkUserDataObject* >::iterator iter = 
		s_loadedObjects_.find( guid );
