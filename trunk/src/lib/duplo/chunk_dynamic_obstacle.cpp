...
void ChunkDynamicObstacle::boundingBox( BoundingBox & bb ) const
{
	BW_GUARD;
	bb = pObstacle()->boundingBox();
}
...
{
	BW_GUARD;
	IF_NOT_MF_ASSERT_DEV( pChunk_->online() )
	{
		return;
	}
...
void ChunkStaticObstacle::lend( Chunk * pLender )
{
	...
	if (pChunk_)
	{
		BoundingBox bb = pObstacle()->boundingBox();
		bb.transformBy( this->worldTransform_ );
...
void ChunkStaticObstacle::boundingBox( BoundingBox & bb ) const
{
	BW_GUARD;
	bb = pObstacle()->boundingBox();
}
	...
	attached_( false )
{
	...
	pSuperModel_->boundingBox( boundingBox_ );
}
...
/*		if (!pModel_->isInWorld())
		...
		BoundingBox bb = pModel_->boundingBox();
