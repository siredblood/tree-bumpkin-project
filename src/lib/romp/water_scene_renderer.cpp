...
void WaterSceneRenderer::deleteUnmanagedObjects()
{
	if (pRT_)
	{
		TextureFeeds::delTextureFeed( "waterScene" );
		pRT_->release();
		pRT_ = NULL;	
	}
}
	...
	eyeUnderWater_( false )
{
	...
	TextureFeeds::addTextureFeed( "waterScene",  feed );
...
bool WaterSceneRenderer::recreate( )
{
	...
	// Render target size is now based on the framebuffer copy size.
	success = pRT_->create( uint(float(FullScreenBackBuffer::renderTarget().width()) *	s_textureSize_),
							uint(float(FullScreenBackBuffer::renderTarget().height()) * s_textureSize_), 
...
//	}
	...
	PROFILER_BEGIN( SpeedTree_drawTrees );
