...
void EditorChunkTerrain::calculateShadows(bool canExitEarly /*=true*/, ProgressTask * task /*= NULL*/)
{
    ...
    for (int32 z = 0; z < (int32)hsm.height(); ++z)
    {
	    ...
	    {
			float chunkX = Terrain::BLOCK_SIZE_METERS*(float)x/(float)(hsm.width()-1);
			float chunkZ = Terrain::BLOCK_SIZE_METERS*(float)z/(float)(hsm.height()-1);
...
bool EditorChunkTerrain::edTransform(const Matrix & m, bool transient)
{
	...
	// Move ourselves into the right chunk.  We have to do this
...
BinaryPtr EditorChunkTerrain::edExportBinaryData()
{
	...
	std::string fname = BWResource::resolveFilename( terrainName );
...
{
	return Vector3( GRID_RESOLUTION, 200.f, GRID_RESOLUTION );
}
...
void EditorChunkTerrain::draw()
{
	...
	{
		if (Options::getOptionInt( "chunkTexture/numLayerWarningShow" ) &&
			PanelManager::pInstance() &&
			PanelManager::instance().currentTool() == "TerrainTexture")
		{
			// Show/hide too-many-textures warning overlay
			int maxNumLayers = Options::getOptionInt( "chunkTexture/numLayerWarning" );
			if ( maxNumLayers > 0 &&
...
{	
	...
	TerrainSettingsPtr pSettings = WorldManager::instance().pTerrainSettings();
