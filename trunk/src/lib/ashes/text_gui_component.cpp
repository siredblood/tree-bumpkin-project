...
TextGUIComponent::TextGUIComponent( BwFont::FontPtr f, PyTypePlus * pType )
...
PyObject * TextGUIComponent::pyNew( PyObject * args )
{
	...
	BwFont::FontPtr pFont = fontName == NULL ? s_defFont() : BwFont::FontManager::instance().get( fontName );
	if ( !pFont )
		pFont = s_defFont();
 ...
 */
BwFont::FontPtr TextGUIComponent::s_defFont()
{
	static BwFont::FontPtr sxf = BwFont::FontManager::instance().get( "default_medium.font" );
	return sxf;
}
...
{
	BwFont::FontPtr pFont = BwFont::FontManager::instance().get( fontName );
	if ( !pFont )
	{
...
const std::string TextGUIComponent::font() const
{
	if ( font_ )
	{
		return BwFont::FontManager::instance().findFontName( font_ );
	}
	else
	{
		return BwFont::FontManager::instance().findFontName( s_defFont() );
	}
}
