...
//change
#include "worldeditor/place/place_public.h"
...
bool EditorChunkModel::load( DataSectionPtr pSection, Chunk * pChunk )
{
	...
	if (!ok)  //没有找到相应的模型，
	{
		...
		Matrix cM = pChunk->transform();
		cM.preMultiply(m);

		std::string guid = pSection->readString("UniqueID");
		std::string path = pSection->readString("resource");
		
		PlacePublic::instance().addUnloadModels( guid, path, cM );
		//change//
