...
/*virtual*/ bool TerrainHoleMap2::load(DataSectionPtr dataSection, std::string *error/* = NULL*/)
{
	...
	if (pHolesData)
	{
		...
		holesMapSize_ = largerPow2( holesSize_ );
...
/**
 *  This gets called to synchronize the DirectX hole map texture with the
 *  image.
 */
...
bool TerrainHoleMap2::holeAt(float x, float z) const
{
	...
	return image_.get( int32((x / BLOCK_SIZE_METERS) * float(width())),
		int32((z / BLOCK_SIZE_METERS) * float(height())) );
}
/**
...
bool TerrainHoleMap2::holeAt( float xs, float zs, float xe, float ze ) const
{
	...
	int32 xStart = int32(floorf((xs / BLOCK_SIZE_METERS) * float(width())));
	int32 xEnd = int32(floorf((xe / BLOCK_SIZE_METERS) * float(width())));

	int32 zStart = int32(floorf((zs / BLOCK_SIZE_METERS) * float(height())));
	int32 zEnd = int32(floorf((ze / BLOCK_SIZE_METERS) * float(height())));
