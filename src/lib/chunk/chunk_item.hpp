...
protected:
	...
	/***
	 * \brief The readUnique Method read the UniqueID from the DataSection
	 *		  if the section had not UniqueID tag, then generate a new UniqueID
	 *		  int the EDITOR verson, the CLIENT version generate a ZERO UNIQUEID instead.
	 * \author Jeff Liang
	 * \date   2009-08-19
	 * 
	 * @param[in] pSection
	 */
	void readUniqueID( DataSectionPtr pSection )
	{
		std::string idStr = pSection->readString( "UniqueID" );

		if( !idStr.empty() )
		{
			uniqueID_ = UniqueID( idStr );
			//INFO_MSG( uniqueID_.toString().c_str() );
		}
		else
		{
			
		#ifdef EDITOR_ENABLED
			uniqueID_ = UniqueID::generate();
		#else
			uniqueID_ = UniqueID::zero();
		#endif
...
#ifdef EDITOR_ENABLED
	/**
	 * \bref The saveUniqueID Method save the UniqueID to the .chunk file
	 * @param[in] pDataSection
	 * @see		  readUniqueID
	 * @note	  call in EDITOR version only
	 */
	void saveUniqueID( DataSectionPtr pSection )
	{
		pSection->writeString( "UniqueID", uniqueID_.toString() );
	}
#endif
...
public:
	...
	/**
	 * \brief Get the UniqueID
	 * @return the UniqueID.
	 * @note if returns UniqueID::zero(), indicates that the UniqueID for 
	 *		 this item	is NOT generated in the CLIENT version
	 * @see readUniqueID for details
	 */
	...
	/**
	 * \brief this method for the client to fectch the Matrix information
	 * of the model
	 * \note for now, only the ChunkModel and ChunkTree override the method
	 * I hope that this method DO NOT conflict with other class!
	 */
	 ...
	 */
	virtual void getBoundBox( BoundingBox & bb ) const
	{
		//BoundingBox bb;
		//return bb;
	}
