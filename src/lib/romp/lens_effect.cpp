 ...
 */
void LensEffect::age( float ageBy )
{
	...
	while (it != end)
	{
		...
		{
			// If ageBy is positive then we want to age the visible flare 
			// (for whatever reason)
			if (ageBy > 0.f)
			{
				newAge = it->second.age() + ageBy;
			}
			else if (rate >= 1.f )
			{
		...
		else
		{
			if (rate != 0.f)
			{
				...
				if (newAge > OLDEST_LENS_EFFECT + 1.f)
				{
					newAge = OLDEST_LENS_EFFECT + 1.f;
				}
			}
			else
			{
