...
{
	BW_GUARD;
	// Remove references to all effects waiting for initialization.
...
ManagedEffectPtr EffectManager::get( const std::string& resourceID )
{
	...
	ManagedEffect* pEffect = pInstance()->find( resourceID );
	if (!pEffect)
	{
		...
		// wait for previous compilation to finish. Note we only give one 
		// warning because Visual Studio gets bogged down by when this message 
		// is output in a tight loop.
		while ( !compileMutex_.grabTry() )
		{
			static bool warned = false;

			if ( !warned )
			{
				WARNING_MSG("EffectManager::get():STALLED Effect compilation of" 
					" '%s' in '%s' thread by other thread.\n",
					resourceID.c_str(),
					g_renderThread ? "Render" : "Background" );
				warned = true;
			}
		}
		...
		{
			pInstance()->add( pEffect, resourceID );
...
{
	BW_GUARD;
	IF_NOT_MF_ASSERT_DEV( pInstance() )
	{
		return;
	}

	// See if we have a copy of the effect already
	ManagedEffectPtr pEffect = pInstance()->find( resourceID );
	if (!pEffect)
	{
		// Create the effect if it could not be found.
		pEffect = new ManagedEffect;
...
bool EffectManager::needRecompile( const std::string& resourceID )
{
	...
	return EffectManager::instance().checkModified( objectName, resourceID );
}
...
void EffectManager::delListener(IListener * listener)
{
	...
	{
		WARNING_MSG( "EffectManager::delListener - "
			"trying to delete an unknown listener\n" );
	}
	this->listenersLock_.give();
}
...
bool EffectManager::hashResource( const std::string& name, MD5::Digest& result )
{
	...
	static StrDigestMap hashCache;

	StrDigestMap::iterator it = hashCache.find( name );
	if (it != hashCache.end())
	{
		result = it->second;
		return true;
	...
	MD5 md5;
	md5.append( data->data(), data->len() );
	md5.getDigest( result );

	hashCache[ name ] = result;
	return true;
}
...
std::string EffectManager::resolveInclude( const std::string& name )
{
	...
	std::string pathname =
		EffectManager::instance().pEffectIncludes()->currentPath() + name;
...
bool EffectManager::finishEffectInits()
{
	...
	while( eit != effectInitQueue_.end()  )
	{
		...
		{
			WARNING_MSG("Effect '%s' not finished initializing this frame.\n",
				(*eit)->resourceID().c_str() );
