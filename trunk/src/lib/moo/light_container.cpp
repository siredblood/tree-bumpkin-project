 ...
 *	Constructor
 */
LightContainer::LightContainer( LightContainerPtr pLC, const BoundingBox& bb, bool limitToRenderable, bool dynamicOnly )
{
	BW_GUARD;
...
void LightContainer::init( LightContainerPtr pLC, const BoundingBox& bb, bool limitToRenderable, bool dynamicOnly )
{
...
/*			for (uint32 i = 0; ( i < pLC->nOmnis() ); i++ )
		...
		spotLights_.clear();
		for (uint32 i = 0; ( i < pLC->nSpots() ) && !( limitToRenderable && spotLights_.size() >= 2 ) ; i++ )
		{
 ...
 */
void LightContainer::addToSelf( LightContainerPtr pLC, const BoundingBox & bb,
...
void LightContainer::commitToFixedFunctionPipeline()
{
	...
	for ( i = 0; i < numDirectionals; i++ )
	{
		...
		light.Direction = Vector3( 0, 0, 0 ) - directionals()[i]->worldDirection();
	...
	for ( i = 0; i < numSpots; i++ )
	{
		...
		light.Direction = Vector3( 0, 0, 0 ) - spots()[i]->worldDirection();
		light.Attenuation0 = 1;
		light.Attenuation1 = 0;
		light.Attenuation2 = 0;
		light.Range = 0;
		light.Theta = spots()[i]->cosConeAngle() * MATH_PI;
		light.Phi = spots()[i]->cosConeAngle() * MATH_PI;
