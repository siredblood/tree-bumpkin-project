...
void Geometrics::drawTriangle(const Vector2 &p1, const Vector2 &p2, 
							  const Vector2 &p3, const Moo::Colour &colour)
{
	CustomMesh<Moo::VertexTL> mesh( D3DPT_TRIANGLESTRIP );
	...
	tlv.pos_.x = p1.x + s_offset.x;
	tlv.pos_.y = p1.y + s_offset.y;
	mesh.push_back( tlv );

	tlv.pos_.x = p2.x + s_offset.x;
	tlv.pos_.y = p2.y + s_offset.y;
	mesh.push_back( tlv );

	tlv.pos_.x = p3.x + s_offset.x;
	tlv.pos_.y = p3.y + s_offset.y;
	mesh.push_back( tlv );
...
void Geometrics::drawTexRect(const Vector2& topLeft, const Vector2& bottomRight, Moo::BaseTexturePtr pTex)
{
	DX::Device* device_ = Moo::rc().device();
	...
	if( device_ )
	{
		...
		Moo::rc().setTexture( 0, pTex->pTexture() );

		mesh.draw();
	}
}

void Geometrics::drawTexRect( const Vector2& topLeft, const Vector2& bottomRight, Moo::BaseTexturePtr pTex, bool alpha )
{
	DX::Device* device_ = Moo::rc().device();
	...
	if( device_ )
	{
		...
		Moo::rc().setRenderState( D3DRS_CULLMODE, D3DCULL_CCW );
		Moo::rc().setRenderState( D3DRS_ALPHATESTENABLE, FALSE );
		Moo::rc().setRenderState( D3DRS_ALPHABLENDENABLE, alpha );
		Moo::rc().setRenderState( D3DRS_DESTBLEND, D3DBLEND_INVSRCALPHA );
		Moo::rc().setRenderState( D3DRS_SRCBLEND, D3DBLEND_SRCALPHA );
		Moo::rc().setRenderState( D3DRS_LIGHTING, FALSE );
		Moo::rc().fogEnabled( false );
...
void Geometrics::createLineMesh( const Vector2& pt1, 
					const Vector2& pt2, 
					const Moo::Colour& colour, 
...
void Geometrics::drawLine( const Vector2& pt1, 
			  const Vector2& pt2, 
			  const Moo::Colour& colour )
{
	CustomMesh<Moo::VertexTL> mesh( D3DPT_LINESTRIP );
	setVertexColour( colour );
	createLineMesh( pt1, pt2, colour, mesh );
	Moo::rc().setRenderState( D3DRS_ZWRITEENABLE, FALSE );
	mesh.draw();
	Moo::rc().setRenderState( D3DRS_ZWRITEENABLE, TRUE );
}
...
{	
	CustomMesh<Moo::VertexTL> mesh( D3DPT_TRIANGLESTRIP );
	setVertexColour( colour );
	createRectMesh( topLeft, bottomRight, colour, mesh );
	Moo::rc().setRenderState( D3DRS_ZWRITEENABLE, FALSE );
	mesh.draw();
	Moo::rc().setRenderState( D3DRS_ZWRITEENABLE, TRUE );
}
...
void Geometrics::drawLine(std::vector<Vector3> vPoints,const Moo::Colour& colour)
{
	int iCount = vPoints.size();
	
	DX::Device *device = Moo::rc().device();
	if (iCount < 2 || device==NULL)
		return;
...
void Geometrics::drawLine(std::vector<Vector3> vPoints, int iSectionSize, const Moo::Colour& colour /* = Moo::Colour */)
{
	if(iSectionSize<=0)
		return;
	...
	DX::Device *device = Moo::rc().device();
	if (iCount < iSectionSize || device==NULL)
		return;
	...
	for( int i = 0; i < iCount-iSectionSize; i+=iSectionSize )
	{
		mesh.clear();
		for(int j=0; j<iSectionSize; j++)
		{
			tlv.pos_ = vPoints[i+j];		
			mesh.push_back( tlv );
		}
		
		//tlv.pos_ = vPoints[i+1];
		//mesh.push_back( tlv );
		//tlv.pos_ = vPoints[i+2];
		//mesh.push_back( tlv );
		//tlv.pos_ = vPoints[i+3];
		//mesh.push_back( tlv );
		tlv.pos_ = vPoints[i];
		mesh.push_back( tlv );
		...
		const Moo::Colour& colour, float offset, float tile, bool alwaysDraw )
{
	...
	lines[2] = Vector3( bb.maxBounds().x, bb.maxBounds().y, bb.minBounds().z );
	lines[3] = Vector3( bb.maxBounds().x, bb.minBounds().y, bb.minBounds().z );
	lines[4] = lines[0];

	lines[5] = Vector3( bb.minBounds().x, bb.minBounds().y, bb.maxBounds().z );
	lines[6] = Vector3( bb.minBounds().x, bb.maxBounds().y, bb.maxBounds().z );
	lines[7] = Vector3( bb.maxBounds().x, bb.maxBounds().y, bb.maxBounds().z );
	lines[8] = Vector3( bb.maxBounds().x, bb.minBounds().y, bb.maxBounds().z );
	lines[9] = lines[5];
	...
	Moo::VertexXYZDUV lv;
	lv.colour_ = colour;
	std::vector< Moo::VertexXYZDUV > vertices;
	...
	Moo::rc().drawPrimitiveUP( D3DPT_LINESTRIP, 4, &vertices[0], sizeof( Moo::VertexXYZDUV ) );
	Moo::rc().drawPrimitiveUP( D3DPT_LINESTRIP, 4, &vertices[5], sizeof( Moo::VertexXYZDUV ) );
	Moo::rc().drawPrimitiveUP( D3DPT_LINESTRIP, 1, &vertices[10], sizeof( Moo::VertexXYZDUV ) );
	Moo::rc().drawPrimitiveUP( D3DPT_LINESTRIP, 1, &vertices[12], sizeof( Moo::VertexXYZDUV ) );
	Moo::rc().drawPrimitiveUP( D3DPT_LINESTRIP, 1, &vertices[14], sizeof( Moo::VertexXYZDUV ) );
	Moo::rc().drawPrimitiveUP( D3DPT_LINESTRIP, 1, &vertices[16], sizeof( Moo::VertexXYZDUV ) );
}
...
//绘制矩形面
void Geometrics::drawRect(Vector3 p1,Vector3 p2, Vector3 p3, Vector3 p4, const Moo::Colour& colour)
{
	DX::Device *device = Moo::rc().device();
...
//绘制多变形面
void Geometrics::drawRegion(std::vector<Vector3> vRegionPt, const Moo::Colour& colour)
{
	int iSize = vRegionPt.size();
	if(iSize<3)
		return;
	...
	Vector3 startpos = vRegionPt[0];
	
	for(int i=1; i<iSize-1; i++ )
	{
		vertex.pos_ = startpos;
		mesh.push_back(vertex);
		vertex.pos_ = vRegionPt[i];
		mesh.push_back(vertex);
		vertex.pos_ = vRegionPt[i+1];
		mesh.push_back(vertex);
	}
	...
	//Moo::rc().setRenderState( D3DRS_ALPHATESTENABLE, TRUE );
	Moo::rc().setRenderState( D3DRS_ALPHABLENDENABLE, TRUE );
	Moo::rc().setRenderState( D3DRS_DESTBLEND, D3DBLEND_INVSRCALPHA );
	Moo::rc().setRenderState( D3DRS_SRCBLEND, D3DBLEND_ONE );
...
void Geometrics::drawRegion(std::vector<Vector3> vRegionPt, Vector3 mousept, const Moo::Colour &colour)
{
	int iSize = vRegionPt.size();
	if(iSize<2)
		return;
...
//绘制角度，FirPt为角度的夹角的点，SecPt、ThirPt为2边的点
void Geometrics::drawAngle(Vector3 FirPt, Vector3 SecPt, Vector3 ThirPt, const Moo::Colour& colour)
{
	DX::Device *device = Moo::rc().device();
	...
	CustomMesh<Moo::VertexXYZL> mesh(D3DPT_TRIANGLELIST);
	Moo::VertexXYZL vertex;
	Moo::Colour cuscolor = colour;
	cuscolor.a = 0;
	
	vertex.colour_ = cuscolor;
	vertex.pos_ = SecPt;
	mesh.push_back( vertex );
	vertex.colour_ = colour;
	vertex.pos_ = FirPt;
	...
	Moo::rc().setRenderState( D3DRS_DESTBLEND, D3DBLEND_INVSRCALPHA );
	Moo::rc().setRenderState( D3DRS_SRCBLEND, D3DBLEND_ONE );
	Moo::rc().setRenderState( D3DRS_SHADEMODE, D3DSHADE_GOURAUD);
