...
void CAboutDlg::OnPaint()
{
	...
	std::string space = " ";
	CString builtOn = L( "WORLDEDITOR/GUI/BIGBANG/VERSION_BUILT", aboutVersionString,
		ToolsCommon::isEval() ? space + L( "WORLDEDITOR/GUI/BIGBANG/EVAL" ) : "",
#ifdef _DEBUG
		space + L( "WORLDEDITOR/GUI/BIGBANG/DEBUG" ),
#else
		"",
#endif
...
public:
	...
	bool openHelpFile( const std::string& name, const std::string& defaultFile )
	{
		...
		//if ( result < 32 )
		//{
		//	Commentary::instance().addMsg( L( "WORLDEDITOR/GUI/BIGBANG/HELP_NOT_FOUND", helpFile, name ), Commentary::WARNING );
		//}
	...
	{
		return true; //openHelpFile( "toolsReferenceGuide" , "content_tools_reference_guide.pdf" );//凸凸去除
	}

	// Open the Content Creation Manual (CCM)
	bool OnContentCreation( GUI::ItemPtr item )
	{
		return true;//openHelpFile( "contentCreationManual" , "content_creation.chm" );//凸凸去除
	}

	bool OnShortcuts( GUI::ItemPtr )
	{
        //ShortcutsDlg::instance()->ShowWindow(SW_SHOW);//凸凸去除
		return true;
	}
};
...
BOOL WorldEditorApp::InitInstance()
{
	...
	{
		ToolsCommon::outOfDateMessage( L("WORLDEDITOR/APPLICATION_NAME") );
		return FALSE;
	}
	...
	WorldEditorApp::instance().m_pszAppName = _strdup( L( "WORLDEDITOR/APPLICATION_NAME" ) );
	...
	//WaitDlg::show( L("WORLDEDITOR/INITIALISING_WORLDEDITOR_MSG") );
	WaitDlg::show(_T("\t\t\t请稍后..."));
	...
	//if (!s_mfApp->init( hInst, m_pMainWnd->m_hWnd,
	//	mainFrame->m_wndSplitter.GetPane(0, 1)->m_hWnd, 
	//	Initialisation::initApp ))
	//{
	//	return FALSE;
	//}
	...
	//PanelManager::instance().disableEnablePanels(NULL);

	updateMailSlot_ = CreateMailslot( "\\\\.\\mailslot\\WorldEditorUpdate", 0, 0, NULL );
	...
	//20110725:默认为obstacleSnap的模式
	WorldManager::instance().obstacleSnapsEnabled();
...
BOOL WorldEditorApp::parseCommandLineMF()
{
	...
	// parse the arguments
	argv[ argc ] = strtok( m_lpCmdLine, sepsArgs );
