...
//TODO : blergh
Matrix ownNormaliseMatrix( const Matrix& m )
{
	Matrix m2 = m;
	m2[0].normalise();
	m2[1].normalise();
	...
	adjFactor_(adjFactor)
{
	...
	selectionMesh_.addDisc( Vector3( 0, 0, 0 ), editableRadius, editableRadius + 0.5f, colour, new RadiusShapePart( Moo::Colour( 1, 0, 0, 0 ), 0 ) );

	invObjectTransform_.invert( ownNormaliseMatrix( objectTransform() ) );
}
...
bool RadiusGizmo::intersects( const Vector3& origin, const Vector3& direction, float& t )
{
	if ( !active_ )
	{
		return false;
	...
	invObjectTransform_.invert( ownNormaliseMatrix( gizmoTransform() ) );
