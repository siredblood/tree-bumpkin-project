 ...
 *	requested resource have previouly been loaded.
 *
 *	@param	id	path to resource
 *
 *	@return		ResourceRef object referencing the requested resource if it
 *				have previouly been loaded. NULL ResourceRef otherwise.
 */
...
{
	CRITICAL_MSG( "Bad\n" );
	BwFont::FontPtr pFont = BwFont::FontManager::instance().get( id );
	return ResourceRef( &*pFont, ResourceRef::stdModRef<BwFont::Font>, id, NULL );
}
