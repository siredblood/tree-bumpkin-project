...
#include "EngineCore/CustomCamera.h"
#include "EngineCore/FlyTravel.hpp"
#include "EngineCore/GPSNavigate.hpp"
// -----------------------------------------------------------------------------
// Section: FreeCamera
// -----------------------------------------------------------------------------
...
{
	BW_GUARD;
	if( CCustomCamera::Instance().IsAroundTar() || CFlyTravel::Instance().IsStartTravel() || 
		CGPSNavigate::Instance().IsStartNavigate()  )
		return false;
...
static const float MOUSE_V_SPEED = 100.0f;
static const float MOUSE_W_SPEED = 15.0f;
...
bool FreeCamera::handleMouseEvent( const MouseEvent & event )
{
	...
	if( CCustomCamera::Instance().IsAroundTar() )
		return false;

	fDz_ = float(event.dz()) / 50.0f;
	...
	float yaw_, pitch_;
	yaw_ = deltaYaw + CameraControl::yawVelocity();
	pitch_ = deltaPitch + CameraControl::pitchVelocity();
	CameraControl::yawVelocity( yaw_ );
	if( pitch_>-1.5707 && pitch_<1.5707)
		CameraControl::pitchVelocity( pitch_ );
	//else
	//	CameraControl::pitchVelocity( CameraControl::pitchVelocity() );

	//INFO_MSG( "%f, %f\n", deltaYaw + CameraControl::yawVelocity(), deltaPitch +
...
bool FreeCamera::handleAxisEvent( const AxisEvent & event )
{
	...
	else if (event.axis() == AxisEvent::AXIS_LX)
	{
		...
		if (InputDevices::isKeyDown(KeyEvent::KEY_JOYALPUSH))
...
void FreeCamera::update( float dTime )
{
	...
	else if ( !fixed_ )
	{
		...
		invView_.postRotateX( CameraControl::pitchVelocity() );
		...
		*/
		...
		// Update the camera matrix.
		Vector3 pos = invView_.applyToOrigin();

		Vector3 dir = invView_.applyToUnitAxisVector( 2 );
		...
		Matrix rot;
		XPMatrixRotationYawPitchRoll( &rot, CameraControl::yawVelocity(), CameraControl::pitchVelocity(), 0.f );

		invView_.setTranslate( pos );
		invView_.preMultiply( rot );
