...
/*virtual*/ bool ChunkStationNode::isValid(std::string &failureMsg) const
{
	...
	if (id() == UniqueID())
    {
        failureMsg = L("CHUNK/INHABITANTS/CHUNK_STATIONNODE/INVALID_ID");
        return false;
    }

    if (graph() == NULL)
    {
        failureMsg = L("CHUNK/INHABITANTS/CHUNK_STATIONNODE/NOT_CONNECTED_TO_GRAPH");
        return false;
    }

    if (links_.size() != preloadLinks_.size())
    {
        failureMsg = L("CHUNK/INHABITANTS/CHUNK_STATIONNODE/LINK_SIZE_UNEQUAL");
        return false;
    }
    ...
    for (size_t i = 0; i < links_.size(); ++i)
    {
        ...
        {
            failureMsg = L("CHUNK/INHABITANTS/CHUNK_STATIONNODE/NOT_MATCH_END_ITEM");
            return false;
        }
        if (myself != this)
        ...
        {
            failureMsg = L("CHUNK/INHABITANTS/CHUNK_STATIONNODE/PRELOAD_IS_MISSING_LINK");
            return false;
        }
        ...
        if (owns)
        {
            ...
            {
                failureMsg = L("CHUNK/INHABITANTS/CHUNK_STATIONNODE/LINK_PRELOAD_WRONG_DIRECTION");
                return false;
            }
            if 
            (
    ...
    )
    {
        ...
        if (!ok)
        {
            failureMsg = L("CHUNK/INHABITANTS/CHUNK_STATIONNODE/LINK_IN_PRELOAD_NOT_IN_LINK");
            return false;
        }
    }
    ...
    {
        failureMsg = L("CHUNK/INHABITANTS/CHUNK_STATIONNODE/NODE_NOT_REGISTER_TO_GRAPH");
        return false;
    }

    if (chunk() == NULL)
    {
        failureMsg = L("CHUNK/INHABITANTS/CHUNK_STATIONNODE/NODE_NOT_IN_CHUNK");
        return false;
    }
