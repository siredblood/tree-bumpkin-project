...
bool MatrixFilter::init()
{
	...
	while( it != end )
	{
		...
		filter.name_ = pFilter->readString( "name", L("WORLDEDITOR/GUI/PAGE_TERRAIN_FILTER/UNKNOWN_FILTER") );
		bool error = false;
		float sum = 0;
		if ( filter.noise_ )
		{
		...
		else
		{
				...
				y != kernels.end(); ++y )
			{
				...
				StringUtils::vectorFromString( row, cols, " \t" );
				int width = cols.size();
		...
		if ( !error )
		{
			...
			if ( !filter.noise_ )
			{
				...
				StringUtils::toLowerCase( kernelSumStr );
