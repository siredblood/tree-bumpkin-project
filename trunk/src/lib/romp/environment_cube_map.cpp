...
//------------------------------------------------------------------------------
EnvironmentCubeMap::EnvironmentCubeMap( uint16 textureSize ) :
	pCubeRenderTargetEffectConstant_( NULL ),
	environmentCubeMapFace_( 0 ),
	numberOfFacesPerFrame_( 1 )
{	
	pCubeRenderTarget_ = new Moo::CubeRenderTarget( "EnvironmentCubeMap" );
...
void EnvironmentCubeMap::update( float dTime, bool defaultNumFaces, uint8 numberOfFaces, uint32 drawSelection )
{
	...
	const Vector3 & center = Moo::rc().world().applyToOrigin();
	...
	else
	{
		ds = EnviroMinder::DrawSelection::sunAndMoon | 
			...
			EnviroMinder::DrawSelection::clouds | 
