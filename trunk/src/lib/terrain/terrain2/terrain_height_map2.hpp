...
namespace Terrain
{
	...
	{
		return heights_.width() - ( internalVisibleOffset() * 2 ) - 1;
	}

	inline uint32 TerrainHeightMap2::internalBlocksHeight() const
	{
		return heights_.height() - ( internalVisibleOffset() * 2 ) - 1;
	}

	inline float TerrainHeightMap2::internalSpacingX() const
	{
		return BLOCK_SIZE_METERS / internalBlocksWidth();
	}

	inline float TerrainHeightMap2::internalSpacingZ() const
	{
		return BLOCK_SIZE_METERS / internalBlocksHeight();
	}
