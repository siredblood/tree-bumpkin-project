	...
	if (useLocPos)
	{
		...
		if ( ( pSection->sectionName() == "model" || pSection->sectionName() == "speedtree" ) )
		{
			...
			if ( largestEdge < 0.01f )
				largestEdge = 0.01f;
			//change 更改大小
			float largestScale = 99999.99f / largestEdge;
			if( largestEdge > 99999.99 )
			{
...
PyObject * ChunkItemPlacer::py_deleteChunkItems( PyObject * args )
{
	...
	for (; i != items.end(); ++i)
	{
		...
		//change 只对userdataobject进行删除
		if ( strcmp( pItem->edClassName(), "ChunkUserDataObject" ) != 0 )
			continue;
...
//change 根据指定模型路径，GUID，坐标，旋转，缩放添加模型
ChunkItemPtr ChunkItemPlacer::createDBChunkItem( const char * namePath, 
												const std::string &modelGUID, const float & posX, 
												const float & posY, const float & posZ, const float & rotX, 
												const float & rotY, const float & rotZ, const float & scaleX, 
												const float & scaleY, const float & scaleZ, bool selected /*= true */ )
{
	Vector3 snaps = WorldManager::instance().movementSnaps();
	...
	//创建Secion
	DataSectionPtr pSection = new XMLSection("model");
	pSection->writeString("resource",namePath);
	...
	//默认不显示阴影
	pItem->edSetCastsShadow(false);

	//GUID如果为空，则系统自行生成
	if (modelGUID != "")
	{
		pItem->edSetGUID(modelGUID);
	}
	else
	{
		pItem->edCreateGUID();
	...
	Matrix pos;
	pos.setTranslate(Vector3(posX,posY,posZ));
	...
	if ( pSection->sectionName() == "model" )
	{
		Matrix rot = Matrix::identity;
		rot.setRotate(DEG_TO_RAD(rotY),DEG_TO_RAD(rotX),DEG_TO_RAD(rotZ));
		newPose.preMultiply( rot ); 

		Matrix scale = Matrix::identity;
		scale.setScale( scaleX,scaleY,scaleZ);
		newPose.preMultiply( scale );
	}
	if (!pItem->edTransform( newPose,false))
	{
		//如果无法在这。。。
...
//ChunkItemPtr ChunkItemPlacer::createDBChunkItem( const char * namePath, const std::string &modelGUID, const Vector3 &pos, const Vector3 &dir, const Vector3 &scale )
//{
//	return createDBChunkItem( namePath, modelGUID, pos.x, pos.y, pos.z, dir.x,dir.y,dir.z, scale.x, scale.y, scale.z );
//}

ChunkItemPtr ChunkItemPlacer::createDBChunkItem( const char * namePath, const std::string &modelGUID, const Vector3 &pos, const Vector3 &dir, const Vector3 &scale, bool selected /*= true */ )
{
	return createDBChunkItem( namePath, modelGUID, pos.x, pos.y, pos.z, dir.x,dir.y,dir.z, scale.x, scale.y, scale.z, selected );
}

ChunkItemPtr ChunkItemPlacer::createDBChunkParticularItem( const char * namePath, 
	const std::string &particularGUID, const float & posX, 
	const float & posY, const float & posZ, const float & rotX, 
	const float & rotY, const float & rotZ, const float & scaleX, 
	const float & scaleY, const float & scaleZ, bool selected /*= true */ )
{
	...
	//创建Secion
	DataSectionPtr pSection = new XMLSection("particles");
	pSection->writeString("resource",namePath);
...
ChunkItemPtr ChunkItemPlacer::createUDOChunkItem( 
	const Matrix& transform_, const std::string& guid_ /*= ""*/, 
	const std::string& chunkID_ /*= ""*/, const std::string& nextGUID_ /*= ""*/, 
	const std::string& nextChunkID_ /*= ""*/, const std::string& backGUID_ /*= ""*/, 
	const std::string& backChunkID_ /*= ""*/, bool selected /*= true */ )
{	
	Vector3 wpos = transform_.applyToOrigin();
	...
	DataSectionPtr pSection = new XMLSection( "UserDataObject" );
	pSection->writeString( "type", "CameraNode" );
	
	if ( !guid_.empty() )
		pSection->writeString( "guid", guid_ );
		
	if ( !nextGUID_.empty() )
		pSection->writeString( "properties/next/guid", nextGUID_ );
		
	if ( !nextChunkID_.empty() )
		pSection->writeString( "properties/next/chunkId", nextChunkID_ );
		
	if ( !backGUID_.empty() )
		pSection->writeString( "backLinks/link/guid", backGUID_ );
		
	if ( !backChunkID_.empty() )
		pSection->writeString( "backLinks/link/chunkId", backChunkID_ );
	...
	Matrix localPose;
	localPose.multiply( transform_, pChunk->transformInverse() );
	Matrix newPose;
	...
	Matrix rot = Matrix::identity;
	rot.setRotate( transform_.yaw(), transform_.pitch(), transform_.roll() );
	newPose.preMultiply( rot ); 
