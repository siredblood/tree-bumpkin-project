 ...
 *  white space.  If these conditions are met, a Matrix will be read in from
 *  the four rows otherwise, a Matrix with all zeros will be created.
 *
 *	If there is more than one section with the specified name, then only the
 *	first one is used.
 *
...
PyObject * PyDataSection::py_createSectionFromString( PyObject * args )
{
	...
	DataSectionPtr newSection = &*XMLSection::createFromStream( "", sstream );
 ...
 *	@components{ all }
 *
 *	This method deletes the section specified by the input path.  If more than
 ...
 *
 *	@param	sectionName		the path to the section to delete.
 *
 *	@return					1 if it was deleted, 0 otherwise.
 */
/**
 *	This method deletes the section at the input path.
 */
PyObject * PyDataSection::py_deleteSection( PyObject * args )
{
	...
	bool result = pSection_->deleteSection( path );
