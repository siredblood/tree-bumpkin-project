 ...
 * 	@see socket(int type)
 */
INLINE Endpoint::Endpoint( bool useSyncHijack ) :
	socket_( NO_SOCKET ),
	useSyncHijack_( useSyncHijack ),
	shouldSendClose_( false ),
	hijackPort_( 0 ),
	hijackAddr_( 0 ),
	pHijackEndpoint_( NULL )
{
}
...
{
	if ((socket_ != NO_SOCKET) && Endpoint::isHijacked())
	{
...
{
	if (Endpoint::isHijacked())
	{
		this->hijackSendOpen( networkPort, networkAddr );

		// Dummy it and bind to the local interface. This is only really needed
		// for the Mercury nub but it shouldn't do much harm doing it for all.
		sockaddr_in	sin;
		sin.sin_family = AF_INET;
		sin.sin_port = 0;
		sin.sin_addr.s_addr = htonl(0x7f000001u);
		MF_VERIFY(
			::bind( socket_, (struct sockaddr*)&sin, sizeof(sin) ) == 0 );
	...
	u_int16_t * networkPort, u_int32_t * networkAddr ) const
{
	if (pHijackEndpoint_)
	{
		// Check that we have this information. We are only getting this
		// information for sockets that use the asynchronous connection. (i.e.
		// the mercury nub and the watcher nub).
		MF_ASSERT( !useSyncHijack_ );

		if (networkPort != NULL) *networkPort = hijackPort_;
		if (networkAddr != NULL) *networkAddr = hijackAddr_;
		return 0;
	}
...
/**
 *	This method sends a message to the front-end process that a new Endpoint has
 *	opened that is being hijacked.
 */
INLINE void Endpoint::hijackSendOpen( u_int16_t port, u_int32_t addr )
{
	MF_ASSERT( Endpoint::isHijacked())


	MF_ASSERT( !shouldSendClose_ );
	shouldSendClose_ = true;

	int16 msgType = HIJACK_MSG_OPEN_UDP;
	int32 fileDescriptor = int(socket_);
	int8 isSync = useSyncHijack_;

	pHijackEndpoint_ =
		useSyncHijack_ ?  s_pHijackEndpointSync_ : s_pHijackEndpointAsync_;

	s_pHijackEndpointSync_->send( &msgType, sizeof( msgType ) );
	s_pHijackEndpointSync_->send( &fileDescriptor, sizeof( fileDescriptor ) );
	s_pHijackEndpointSync_->send( &isSync, sizeof( isSync ) );
	s_pHijackEndpointSync_->send( &port, sizeof( port ) );
	s_pHijackEndpointSync_->send( &addr, sizeof( addr ) );

	// As an optimisation, the address of the associated front-end socket is
	// only received for those sockets that use the asynchronous connection.
	// This should be the mercury and watcher nubs.
	if (!useSyncHijack_)
	{
		int32 fd;

		if (!s_pHijackEndpointSync_->recvAll( &msgType, sizeof( msgType ) ) ||
			(msgType != HIJACK_MSG_OPEN_UDP) ||
			!s_pHijackEndpointSync_->recvAll( &fd, sizeof( fd ) ) ||
			(fd != socket_) ||
			!s_pHijackEndpointSync_->recvAll( &hijackAddr_, sizeof( hijackAddr_ ) ) ||
			!s_pHijackEndpointSync_->recvAll( &hijackPort_, sizeof( hijackPort_ ) ) )
		{
			// Open failed!!
		}
	}
...
/**
 *	This method sends a message to the front-end process that an Endpoint that
 *	was being hijacked has closed.
 */
INLINE void Endpoint::hijackSendClose()
{
	MF_ASSERT( s_pHijackEndpointSync_ );
	if (shouldSendClose_)
	{
		shouldSendClose_ = false;
		int32 fileDescriptor = int(socket_);
		int16 msgType = HIJACK_MSG_CLOSE_UDP;
		s_pHijackEndpointSync_->send( &msgType, sizeof( msgType ) );
		s_pHijackEndpointSync_->send( &fileDescriptor, sizeof( fileDescriptor ) );
	}
}
	...
	struct sockaddr_in & sin )
{
	if (pHijackEndpoint_)
	{
		// If this Endpoint is hijacked, send an artificial data gram over the
		// hijack connection.
		int32 fileDescriptor = int(socket_);
		int16 msgType = HIJACK_MSG_UDP;
		pHijackEndpoint_->send( &msgType, sizeof( msgType ) );
		pHijackEndpoint_->send( &fileDescriptor, sizeof( fileDescriptor ) );
		pHijackEndpoint_->send( &sin.sin_addr.s_addr, 4 );
		pHijackEndpoint_->send( &sin.sin_port, 2 );
		pHijackEndpoint_->send( &gramSize, sizeof( int32 ) );
		return pHijackEndpoint_->send( gramData, gramSize );
	}
	...
	struct sockaddr_in & sin )
{
	if (s_pHijackStream_)
	{
		...
		u_int32_t addr;
		u_int16_t port;
		(*s_pHijackStream_) >> (uint32&)addr >> (uint16&)port;

		sin.sin_port = port;
		sin.sin_addr.s_addr = addr;

		if (s_pHijackStream_->error())
		{
			return -1;
		}

		int len = std::min( gramSize, s_pHijackStream_->remainingLength() );

		::memcpy( gramData, s_pHijackStream_->retrieve( len ), len );

		MF_ASSERT( len >= 0 );

		return len;
	}
	else if (s_pHijackEndpointSync_)
	{
#ifdef _WIN32
...
INLINE int Endpoint::getInterfaceAddress( const char * name, u_int32_t & address )
{
	if (Endpoint::isHijacked())
	{
		// If this process is acting as a back-end for a front-end process, the
		// network interfaces of the front-end process are considered.
		FrontEndInterfaces::iterator iter = s_frontEndInterfaces_.find( name );

		if (iter != s_frontEndInterfaces_.end())
		{
			address = iter->second;
			return 0;
		}
...
INLINE int Endpoint::getInterfaceFlags( char * name, int & flags )
{
	if (!strcmp(name,"eth0"))
	{
		flags = IFF_UP | IFF_BROADCAST | 0x20 |
			0x40 | IFF_MULTICAST;
		return 0;
	}
	else if (!strcmp(name,"lo"))
	{
		flags = IFF_UP | IFF_LOOPBACK | 0x40;
		return 0;
	}
	return -1;
}
