 ...
 *	The format string should not be longer than 512 bytes including the null
 *	terminator and contents.
 *
 ...
 *						identifier.
 *	@param format		A format string similar to that used by printf() to
 *						help format the output.
 *
 *	@return	True if it was successful, False otherwise.
 */
bool DataSection::writeBool( const std::string &tagPath,
		bool value,
		const char *format )
{
	...
	if(pSection)
		return pSection->setBool(value, format);
	else
		return false;
}
 ...
 *	@return	True if it was successful, False otherwise.
 */
bool DataSection::writeInt( const std::string &tagPath,
		int value,
		const char *format )
{
	DataSectionPtr pSection = this->openSection(tagPath, true);
	...
	if(pSection)
		return pSection->setInt(value, format);
	else
		return false;
}
 ...
 *	@return	True if it was successful, False otherwise.
 */
bool DataSection::writeLong( const std::string &tagPath,
		long value,
		const char *format )
{
	DataSectionPtr pSection = this->openSection(tagPath, true);
	...
	if(pSection)
		return pSection->setLong(value, format);
	else
		return false;
}
 ...
 *	@return	True if it was successful, False otherwise.
 */
bool DataSection::writeFloat( const std::string &tagPath,
		float value,
		const char *format )
{
	DataSectionPtr pSection = this->openSection(tagPath, true);
	...
	if(pSection)
		return pSection->setFloat(value, format);
	else
		return false;
}
 ...
 *	@return	True if it was successful, False otherwise.
 */
bool DataSection::writeDouble( const std::string &tagPath,
		double value,
		const char *format )
{
	DataSectionPtr pSection = this->openSection(tagPath, true);
	...
	if(pSection)
		return pSection->setDouble(value, format);
	else
		return false;
}
...
/**
 *	This method writes a string value to the tag specified. The
 *	write(Wide)String methods are the only methods with no format
...
/**
 *	This method writes a wide string value to the tag specified. The
 *	write(Wide)String methods are the only methods with no format string
 *	in the parameter list.
 *	This means that the string will have to be formatted before calling this
 *	method.
 *
 *	@param tagPath		The name/path of the identifier to be written to.
 *	@param value		The string value that will be associated with that
		...
		const char *format )
{
	...
	if(pSection)
		return pSection->setVector2(value, format);
	else
		return false;
}
		...
		const char *format )
{
	...
	if(pSection)
		return pSection->setVector3(value, format);
	else
		return false;
}
		...
		const char *format )
{
	...
	if(pSection)
		return pSection->setVector4(value, format);
	else
		return false;
}
 ...
 *	The format string should not be longer than 2048 bytes including the null
 *	terminator and contents.
 *
 *	@param tagPath		The name/path of the identifier to be written to.
		...
		const char *format )
{
	...
	if(pSection)
		return pSection->setMatrix34(value, format);
	else
		return false;
}
 ...
 *	String will have to be formatted before calling this
 *	method.
 *
 *	@param tagPath		The name/path of the identifier to be written to.
 *	@param value		The string value that will be associated with that
 ...
 *	@param format		A format string similar to that used by printf() to
 *						help format the output. This is applied to each
 *						individual element in the vector.
 *
 *	@return	None.
 */
void DataSection::writeInts( const std::string &tagPath,
		std::vector<int> &src, int /*flags*/, const char* format)
{
...
void DataSection::writeBools( const std::string &tagPath,
		std::vector<bool> &src, int /*flags*/, const char* format)
{
	DataSectionPtr pSection, pNewSection;
	std::string tag;
...
void DataSection::writeLongs( const std::string &tagPath,
		std::vector<long> &src, int /*flags*/, const char* format)
{
	DataSectionPtr pSection, pNewSection;
	std::string tag;
...
void DataSection::writeFloats( const std::string &tagPath,
		std::vector<float> &src, int /*flags*/, const char* format)
{
	DataSectionPtr pSection, pNewSection;
	std::string tag;
	...
	{
		pNewSection = pSection->newSection(tag);
		pNewSection->setFloat(*it, format);
	}
}
...
void DataSection::writeDoubles( const std::string &tagPath,
		std::vector<double> &src, int /*flags*/, const char* format)
{
	DataSectionPtr pSection, pNewSection;
	std::string tag;
	...
	{
		pNewSection = pSection->newSection(tag);
		pNewSection->setDouble(*it, format);
	}
}
...
void DataSection::writeVector2s( const std::string &tagPath,
		std::vector<Vector2> &src, int /*flags*/, const char* format)
{
	DataSectionPtr pSection, pNewSection;
	std::string tag;
	...
	{
		pNewSection = pSection->newSection(tag);
		pNewSection->setVector2(*it, format);
	}
}
...
void DataSection::writeVector3s( const std::string &tagPath,
		std::vector<Vector3> &src, int /*flags*/, const char* format)
{
	DataSectionPtr pSection, pNewSection;
	std::string tag;
	...
	{
		pNewSection = pSection->newSection(tag);
		pNewSection->setVector3(*it, format);
	}
}
...
void DataSection::writeVector4s( const std::string &tagPath,
		std::vector<Vector4> &src, int /*flags*/, const char* format)
{
	DataSectionPtr pSection, pNewSection;
	std::string tag;
	...
	{
		pNewSection = pSection->newSection(tag);
		pNewSection->setVector4(*it, format);
	}
}
 ...
 *						affects whether the previous vector information is
 *						deleted first or simply added to.
 *	@param vectorFormat	A format string similar to that used by printf() to
 *						help format the output. This is applied to each
 *						individual element in the vector.
 *
 *	@return	None.
...
void DataSection::writeMatrix34s( const std::string &tagPath,
		std::avector<Matrix> &src, int /*flags*/, const char* vectorFormat)
{
	DataSectionPtr pSection, pNewSection;
	std::string tag;
	...
	{
		pNewSection = pSection->newSection(tag);
		pNewSection->setMatrix34(*it, vectorFormat);
	}
}
...
bool DataSection::setBool( bool /*value*/, const char * /*format*/ )
{
	return false;
}

bool DataSection::setInt( int /*value*/, const char * /*format*/ )
{
	return false;
}

bool DataSection::setLong( long /*value*/, const char * /*format*/ )
{
	return false;
}

bool DataSection::setFloat( float /*value*/, const char * /*format*/ )
{
	return false;
}

bool DataSection::setDouble( double /*value*/, const char * /*format*/ )
{
	return false;
}
...
bool DataSection::setVector2( const Vector2 & /*value*/, const char * /*format*/ )
{
	return false;
}

bool DataSection::setVector3( const Vector3 & /*value*/, const char * /*format*/ )
{
	return false;
}

bool DataSection::setVector4( const Vector4 & /*value*/, const char * /*format*/ )
{
	return false;
}
