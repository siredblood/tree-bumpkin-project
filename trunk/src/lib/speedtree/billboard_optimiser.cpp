...
const float BillboardVertex::s_minAlpha_ = 84.0f;
const float BillboardVertex::s_maxAlpha_ = 255.0f;
...
void saveTextureAtlas( bool save, DX::BaseTexture * texture )
{
	if ( save )
	{
		HRESULT hresult = D3DXSaveTextureToFile(
			"bboptimiser.dds", D3DXIFF_DDS, texture, NULL );
	}
}
