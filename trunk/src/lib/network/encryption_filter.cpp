...
bool EncryptionFilter::initKey()
{
	...
	if ((keySize_ >= MIN_KEY_SIZE) && (keySize_ % BLOCK_SIZE == 0))
	{
 ...
 */
Reason EncryptionFilter::send( Nub & nub, const Address & addr,
		Packet * pPacket )
{
	int len = 0;
	...
	return this->PacketFilter::send( nub, addr, toSend.getObject() );
}
