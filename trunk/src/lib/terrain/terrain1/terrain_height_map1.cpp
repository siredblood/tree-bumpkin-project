...
{
    return BLOCK_SIZE_METERS/blocksWidth();
}
...
{
    return BLOCK_SIZE_METERS/blocksHeight();
}
...
) const
{
	...
	BoundingBox bb( Vector3( 0, minHeight(), 0 ), 
		Vector3( BLOCK_SIZE_METERS, maxHeight(), 
        BLOCK_SIZE_METERS ) );

	const Vector3 gridAlign( float(blocksWidth()) / BLOCK_SIZE_METERS, 1.f, 
		float(blocksHeight()) / BLOCK_SIZE_METERS );
	...
	if (bb.clip(s, e, BOUNDING_BOX_EPSILON))
	{
		...
		if (gridX == int32(blocksWidth()) && gridDirX == 0 &&
			almostEqual( BLOCK_SIZE_METERS, source.x, COLLISION_EPSILON ) )
			--gridX;
		
		// Do boundary check, if we are at the very edge of the block,
		// check against the edge triangles rather than one past the edge
		if (gridZ == int32(blocksHeight()) && gridDirZ == 0 &&
			almostEqual( BLOCK_SIZE_METERS, source.z, COLLISION_EPSILON ))
			--gridZ;
