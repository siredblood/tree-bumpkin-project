...
bool Flora::init( DataSectionPtr pSection, uint32 terrainVersion )
{
	...
	this->maxVBSize_ = pSection->readInt( "vbSize", s_maxVBSize );
	...
	maxSlope_ = pSection->readFloat( "maxSlope", 30.f );
...
void Flora::createUnmanagedObjects()
{
	...
	this->vbSize(uint32(this->maxVBSize_ * ratio));
}
...
Ecotype::ID Flora::generateEcotypeID( const Vector2& p )
{
	...
	if (details.pBlock_ && details.pBlock_->dominantTextureMap())
	{
		...
		Vector3 normal = 
			pBlock->normalAt( relPos.x, relPos.z );
		float slope = RAD_TO_DEG(acosf(Math::clamp(-1.0f, normal.y, +1.0f)));
		if (slope < maxSlope_)
		{
...
//		return;
	...
	//we need to unlock the vertex buffer now.
	pRenderer_->postUnlock();
...
{
	Vector3 pos( terrainBlockTransform.applyToOrigin() );
	pos /= 100.f;
	int x = abs((int)(floorf(pos.x + 0.5f)));
	int z = abs((int)(floorf(pos.z + 0.5f)));
	return ( x%2 | (z%2 << 1) );
}


const char* Flora::groundType( float x, float z )
{
	Ecotype& e = ecotypeAt( Vector2(x,z) );
	return e.soundTag().c_str();
}
