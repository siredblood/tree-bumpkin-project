...
/**
 * This method locks the vb
 * @return pointer to the first byte in the vb
 */
BYTE* FloraRenderer::lockVB( )
{
	BYTE* pData = NULL;
	if (pVB_.valid())
	{
        if (FAILED( pVB_.lock( 0, 0, (void**)&pData, lockFlags_ )))
		{
			pData = NULL;
			//ERROR_MSG( "FloraRenderer::lockVB - unable to lock vertex buffer\n" );
		}
	}
	return pData;
}
...
/**
 * This method unlocks the vb
 */
void FloraRenderer::unlockVB( )
{
    if (FAILED( pVB_.unlock() ))
	{
		//ERROR_MSG( "FloraRenderer::unlockVB - failed trying to unlock VB\n" );
	}
}
...
FloraVertexContainer* FloraRenderer::lock( uint32 firstVertex, uint32 nVertices )
{
	if (!pContainer_)
	{
		...
		FloraVertex* pVerts = (FloraVertex*)pLocked_;
		if (pVerts)
		{
			pVC->init( pVerts + firstVertex, nVertices );
			pContainer_ = pVC;
		}
		return pContainer_;
	}
...
{		
	...
	{				
		...
		Vector4 vertexToWorld( Vector4(tr.applyToOrigin(),0.f) );
		parameters_->setVector( "vertexToWorld", &vertexToWorld );
...
bool FloraRenderer::preLock()
{
	MF_ASSERT( pLocked_ == NULL );
	pLocked_ = lockVB();
	return ( pLocked_ ? true : false );
}
