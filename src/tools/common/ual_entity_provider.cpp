...
public:
	...
	static AssetInfo assetInfo( int index )
	{
		...
		std::string definedProviderFile = BWResource::resolveFilename(
			getDefDirectory() + name + ".def" );
		
		// UAL uses back-slashes, as windows
		std::replace( definedProviderFile.begin(), definedProviderFile.end(), '/', '\\' );
		return AssetInfo( getAssetName(), name, definedProviderFile );
	}
...
{
public:
	bool isValid( const ThumbnailManager& manager, const std::string& file )
	{
	...
	bool needsCreate( const ThumbnailManager& manager, const std::string& file, std::string& thumb, int& size )
	{
		thumb = imageFile_;
		return false;
	}

	bool prepare( const ThumbnailManager& manager, const std::string& file )
	{
	...
	bool render( const ThumbnailManager& manager, const std::string& file, Moo::RenderTarget* rt  )
	{
		// should never get called
		MF_ASSERT( false );
		return false;
...
std::string EntityThumbProv< ENTITY_PROVIDER >::imageFile_;

IMPLEMENT_THUMBNAIL_PROVIDER( EntityThumbProv< USER_DATA_OBJECT_PROVIDER > )
std::string EntityThumbProv< USER_DATA_OBJECT_PROVIDER >::imageFile_;
...
template <typename providerMap>
const std::string EntityVFolderProvider<providerMap>::getDescriptiveText( VFolderItemDataPtr data, int numItems, bool finished )
{
	if ( !data )
		return "";
	...
	if ( data->isVFolder() )
	{
		...
		return L("COMMON/UAL_ENTITY_PROVIDER/ENTITIES", providerMap::size());
	}
	else
	{
