...
void FlarePSA::execute( ParticleSystem &particleSystem, float dTime )
{
	...
	if (flareStep_ == 0)
	{
		...
		if (particleSystem.size() > 0)
		{
			...
			const Particle* particle = particleSystem.particles()->lastAdded();
			...
			LensEffectManager::instance().add( ParticleSystem::getUniqueParticleID( *particle, particleSystem ), lePosition, le_ );
			particleSystem.addFlareID( ParticleSystem::getUniqueParticleID( *particle, particleSystem ) );
		}
	}
	else if (flareStep_ > 0)
	{
		...
		while (current != endOfParticles)
		{
			...
			if (particle.isAlive())
			{
				...
				if (particle.index() % flareStep_ == 0)
				{
					if (useParticleSize_ && !isMesh)
					{
						le_.size( particle.size() );
