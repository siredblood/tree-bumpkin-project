...
bool TerrainRenderer2::initInternal( DataSectionPtr pSettingsSection )
{
	...
	// init watchers
		...
		MF_WATCH( "Render/Terrain/Terrain2/Draw Heightmap", drawHeightmap,
			Watcher::WT_READ_WRITE,
			"Enable visualization of terrain height map.");
											...
											bool			lowDetail)
{
	...
	if ( lowDetail )
	{
		...
		Vector3 v[4];
		v[0].set( 0, h[0], 0 );
		v[1].set( 0, h[1], BLOCK_SIZE_METERS );
		v[2].set( BLOCK_SIZE_METERS, h[2], 0 );
		v[3].set( BLOCK_SIZE_METERS, h[3], BLOCK_SIZE_METERS );
	...
	else
	{
		...
		uint32	size = hmp->verticesWidth();
		float	step = BLOCK_SIZE_METERS / float(size);
				...
				else
				{
					...
					if ( tb2->pHolesMap() )
					{
						if ( tb2->depthPassMark() != Moo::rc().renderTargetCount() )
							drawZPass = true;
						if ( Moo::rc().getRenderState( D3DRS_FILLMODE ) == D3DFILL_WIREFRAME )
							drawZPass = true;
						if ( zBufferIsClear_ )
							drawZPass = true;
					}
											...
											uint32&					passCount )
{
	...
	DWORD oldColourWrite;
	DWORD oldColourWrite1;
	if ( drawMethod == DM_Z_ONLY )
	{
		oldColourWrite = Moo::rc().getRenderState( D3DRS_COLORWRITEENABLE );
		oldColourWrite1 = Moo::rc().getRenderState( D3DRS_COLORWRITEENABLE1 );
	...
	bool doStencilDraw = !Moo::rc().reflectionScene() && useStencilMasking_ &&
		Moo::rc().getRenderState( D3DRS_FILLMODE ) != D3DFILL_WIREFRAME;
	...
	if ( pEffectMaterial->begin() )
	{
		for ( uint32 i = 0; i < pEffectMaterial->nPasses(); i++ )
		{
			if ( pEffectMaterial->beginPass(i) )
			{
				...
				uint32 stencilOn = Moo::rc().getRenderState(D3DRS_STENCILENABLE);		
				...
				Moo::rc().setRenderState(D3DRS_STENCILENABLE, stencilOn);
	...
	if ( drawMethod == DM_Z_ONLY )
	{
		...
		Moo::rc().setWriteMask( 0, oldColourWrite );
		Moo::rc().setWriteMask( 1, oldColourWrite1 );
	}
