...
{
	BW_GUARD;
	for (uint o = 0; o < obstacles_.size(); o++)
	{
		this->delFromSpace( *obstacles_[o] );
	}
}
...
int ChunkModelObstacle::focus()
{
	...
	for (uint o = 0; o < obstacles_.size(); o++)
	{
		foco += this->addToSpace( *obstacles_[o] );
	}
...
void ChunkModelObstacle::delObstacles( ChunkItemPtr pItem )
{
	...
	{
		ChunkObstacle & cso = *obstacles_[i];
		if (cso.pItem() == pItem)
		{
			...
			// the chunkobstacle will be deleted
			// when its reference count reaches zero.

			// ok it's removed. Don't skip the next one
			i--;
		}
	}
}
