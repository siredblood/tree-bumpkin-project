...
ProgressDisplay::ProgressDisplay( BwFont::FontPtr pFont, ProgressCallback pCallback, uint32 colour ):
	pFont_( pFont ),
	...
	rowHeight_( 0.05f )
{
	...
	if ( !pFont_ )
	{
		...
		pFont_ = BwFont::FontManager::instance().get( "default_medium.font" );
	}
...
bool ProgressDisplay::draw( bool force )
{
	...
	if ( pFont_ )
	{
		BwFont::FontManager::instance().setMaterialActive( pFont_ );
