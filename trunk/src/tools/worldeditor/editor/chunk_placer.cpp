...
Chunk* ChunkPlacer::utilCreateChunk( DataSectionPtr pDS, const std::string& chunkID, Matrix* m/* = NULL*/ )
{
	...
	Chunk * pChunk = new Chunk( chunkID, WorldManager::instance().chunkDirMapping() );
...
PyObject * ChunkPlacer::py_createChunk( PyObject * args )
{
	...
	UndoRedo::instance().barrier(
		L("WORLDEDITOR/WORLDEDITOR/CHUNK/CHUNK_PLACER/CREATE_CHUNK", pChunk->identifier() ), false );
...
PyObject * ChunkPlacer::py_saveChunkTemplate( PyObject * args )
{
	...
	if( !gotOneShell )
	{
		WorldManager::instance().addCommentaryMsg( L("WORLDEDITOR/WORLDEDITOR/CHUNK/CHUNK_PLACER/SHELL_ONLY" ) );
	}
	else
		WorldManager::instance().addCommentaryMsg( L("WORLDEDITOR/WORLDEDITOR/CHUNK/CHUNK_PLACER/TEMPLATE_SAVED" ) );
...
PyObject* ChunkPlacer::py_cloneAndAutoSnap( PyObject * args )
{
	...
	*/
	...
	if (autoSnapTransform != chunk->transform())
	{
		...
		// set a meaningful barrier name
		UndoRedo::instance().barrier(
			L("WORLDEDITOR/WORLDEDITOR/CHUNK/CHUNK_PLACER/AUTO_CLONE_CHUNK", pChunk->identifier() ), false );
...
std::string ChunkPlacer::utilCreateInsideChunkName( Chunk * pNearbyChunk )
{
	...
	if ( userTag.length() == 2 )
	{
		if ( space != "" )
		{
			...
			{
				bw_snprintf( chunkName, sizeof(chunkName), "%s00%s%04xi.chunk",
					spacesubdir.c_str(), userTag.c_str(), lastNumber );
				bw_snprintf( backupChunkName, sizeof(backupChunkName), "%s00%s%04xi.~chunk~",
					spacesubdir.c_str(), userTag.c_str(), lastNumber );
				++lastNumber;
				...
				if ( !BWResource::openSection( filePath ) && !BWResource::openSection( backupFilePath ) )
				{
					result = chunkName;
					found = true;
