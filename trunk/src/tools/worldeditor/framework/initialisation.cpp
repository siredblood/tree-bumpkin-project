 ...
 */
bool Initialisation::initApp( HINSTANCE hInstance, HWND hWndInput, HWND hWndGraphics )
{
	...
	if (!InputDevices::instance().init( hInstance, hWndInput,
			deviceInitFlags ))
	{
	...
	if ( !WorldManager::instance().init( s_hInstance, s_hWndInput, s_hWndGraphics ) )
	{
		return false;
	}
...
*/
	...
	Moo::rc().fogNear( 100.f ); /*5000.f*/
	Moo::rc().fogFar( 300.f );	/*10000.f*/
	...
	BwFont::FontManager::instance().preCreateAllFonts();
...
bool Initialisation::initConsoles()
{
	...
	mgr.add( pStatisticsConsole,		"Default",		KeyEvent::KEY_F5, MODIFIER_CTRL );
	mgr.add( pResourceConsole,			"Resource",		KeyEvent::KEY_F5, MODIFIER_CTRL | MODIFIER_SHIFT );
	mgr.add( new DebugConsole(),		"Debug",		KeyEvent::KEY_F7, MODIFIER_CTRL );
