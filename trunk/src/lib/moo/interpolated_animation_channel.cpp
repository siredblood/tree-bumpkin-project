...
void InterpolatedAnimationChannel::preCombine( const AnimationChannel & rOther )
{
	...
	Matrix m;
	Matrix us;
	for (PositionKeyframes::iterator it = positionKeys_.begin();
		it != positionKeys_.end();
		it++)
	{
		rOther.result( it->first, m );
		this->result( it->first, us );
		us.preMultiply( m );
		hack.addKey( it->first, us );
	}
...
void InterpolatedAnimationChannel::postCombine( const AnimationChannel & rOther )
{
		...
		it++)
	{
		...
		this->result( it->first, us );	// apply our rot + scale to the pos first

		it->second += us.applyVector( m.applyToOrigin() );
	}

	for (ScaleKeyframes::iterator it = scaleKeys_.begin();
		it != scaleKeys_.end();
		it++)
	{
		rOther.result( it->first, m );
		...
		Quaternion usrot;
		interpolateKeys( rotationKeys_, rotationIndex_, D3DXQuaternionSlerp, it->first, usrot );
		D3DXQuaternionRotationMatrix( &usrot, &us );
		m.postMultiply( us );			// apply our rot to the scale first

		it->second[0] *= row0->length();
		it->second[1] *= row1->length();
		it->second[2] *= row2->length();

		if (row0->crossProduct( *row1 ).dotProduct( *row2 ) < 0)
			it->second[2] *= -1.f;
	}

	for (RotationKeyframes::iterator it = rotationKeys_.begin();
		it != rotationKeys_.end();
		it++)
	{
		rOther.result( it->first, m );
		...
		static_cast<D3DXQUATERNION&>(it->second) = q * it->second;
	}
#endif
}
