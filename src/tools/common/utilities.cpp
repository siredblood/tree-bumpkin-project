...
/*static*/ std::string Utilities::memorySizeToStr( uint32 memorySizeInBytes )
{
    ...
    {
        case 0:
            result = L("COMMON/UTILITIES/BYTES", Formatter( memorySize, "%.1f") );
            break;
        case 1:
            result = L("COMMON/UTILITIES/KILOBYTES", Formatter( memorySize, "%.1f") );
            break;
        case 2:
            result = L("COMMON/UTILITIES/MEGABYTES", Formatter( memorySize, "%.1f") );
            break;
        case 3:
			result = L("COMMON/UTILITIES/GIGABYTES", Formatter( memorySize, "%.1f") );
            break;
        case 4:
            result = L("COMMON/UTILITIES/TERABYTES", Formatter( memorySize, "%.1f") );
            break;
    }
...
)
{
	...
	IFileSystem::Directory dir = BWResource::instance().fileSystem()->readDirectory( spaceFolder );
	for( IFileSystem::Directory::iterator iter = dir.begin(); iter != dir.end(); ++iter )
	{
		if( iter->length() == 15 &&// ????????i.chunk
			iter->substr( 8 ) == "i.chunk" )
			result.insert( iter->substr( 0, 9 ) );
	}
