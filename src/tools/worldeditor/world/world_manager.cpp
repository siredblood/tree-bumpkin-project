...
//change
static DogWatch s_loadUnloadModel( "load_UnloadModel" );
...
void WorldManager::update( float dTime )
{
	...
	if ( acceptInput && castRay )
	{
		...
		//TRACE("%f,%f,%f",worldRay_.x,worldRay_.y,worldRay_.z);
	...
	//ITaskManager
	TaskManager::instance().tick( dTime_ );
...
{
	//Panel 0 - memory load
	this->setStatusMessage( 1, L( "WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/MEMORY_LOAD", getMemoryLoad() ) );
	...
	if ( s_countDown < 0.f )
	{
		this->setStatusMessage( 2, L( "WORLDEDITOR/WORLDEDITOR/BIGBANG/BIG_BANG/FPS",
...
void WorldManager::setReadOnlyFog()
{
	...
	//Moo::Colour colour( (FLOAT)82, (FLOAT)174, (FLOAT)250, (FLOAT)255 );
...
bool WorldManager::canClose( const std::string& action )
{
 ...
 //       }
	...
	//CSailBoatManage::Instance().ReleaseObject();
	...
	UndoRedo::Operation( 0 ), before_( before ), after_( after )
{
	for( std::vector<ChunkItemPtr>::iterator iter = before_.begin(); iter != before_.end(); ++iter )
	{
		addChunk( (*iter)->chunk() );
		//取消选中模型颜色
		(*iter)->setDrawSpecialSurface( false );
	}
	...
	for( std::vector<ChunkItemPtr>::iterator iter = after_.begin(); iter != after_.end(); ++iter )
	{
		addChunk( (*iter)->chunk() );
		//设置选中模型颜色
		(*iter)->setDrawSpecialSurface( true );
	}
