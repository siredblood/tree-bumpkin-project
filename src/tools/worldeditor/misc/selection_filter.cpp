								...
								bool ignoreCameraChunk )
{
	...
	//change 进入自定义筛选状态
	//if ( SelectionFilter::instance().customMode_ )
	//{
	//	//只对sets目录下的模型起作用
	//	if ( !CommonPublic::instance().containtString(item->pOwnSect()->readString("resource"),"sets/") )
	//		return false;
	//}
	...
	if (isShellModel)
	{
		...
		if (!ignoreCurrentSelection && WorldManager::instance().isItemInChunkSelected( item->chunk() ))
			return false;
	}
	else
	{
...
//change 2010-9-9
SelectionFilter & SelectionFilter::instance()
{
	static SelectionFilter s_instance;
	return s_instance;
}
...
/*~ function WorldEditor.getUserMode
 *	@components{ worldeditor }
 *
 *	获取当前的系统操作模式（用户模式/编辑模式)
 *
 *	@return 返回操作模型状态,1为用户模式，0为编辑模式
 */
static PyObject *py_getUserMode( PyObject *args )
{
	bool canEdit_ = g_bUserMode;
	return Py_BuildValue( "i", canEdit_ ? 1 : 0 );
}
PY_MODULE_FUNCTION( getUserMode, WorldEditor )
