...
)
{
    ...
    if (forceToDisk && getInfo.chunkTerrain_ == NULL)
    {
		...
		xform.setTranslate(gx*Terrain::BLOCK_SIZE_METERS, 0.0f, 
							gy*Terrain::BLOCK_SIZE_METERS);
		getInfo.block_->rebuildLodTexture(xform);
        if ( !getInfo.block_->save(space + '/' + getInfo.chunkName_ + ".cdata") )
        {
    ...
    if (getInfo.chunk_ != NULL)
    {
		...
		if (forceToDisk && getInfo.chunkTerrain_ == NULL)
		{
			WorldManager::instance().changedTerrainBlockOffline(getInfo.chunkName_);
		}
		else
		{
