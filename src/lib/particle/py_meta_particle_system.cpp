 ...
 *
 */
PyObject* PyMetaParticleSystem::createBG( const std::string& filename, PyObject * callback )
{
	BW_GUARD;
	MPSBackgroundTaskPtr pTask = new MPSBackgroundTask( filename, callback );
 ...
 */
void PyMetaParticleSystem::boundingBoxAcc( BoundingBox & bb, bool skinny )
{
	BW_GUARD;
	pSystem_->boundingBoxAcc( bb, skinny );	
}
 ...
 *	This method overrides PyAttachment and delegates directly to our system.
 */
void PyMetaParticleSystem::visibilityBoxAcc( BoundingBox & bb )
{
	BW_GUARD;
	pSystem_->visibilityBoxAcc( bb );
}
