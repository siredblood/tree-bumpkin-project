...
public:
	...
	MaskResult *mask( EditorChunkTerrainPtr terrain );

	void changedLayers( EditorChunkTerrainPtr terrain );

	TerrainPaintBrushPtr paintBrush() const;
	void paintBrush( TerrainPaintBrushPtr brush );

protected:
	typedef std::map< EditorChunkTerrainPtr, MaskResult > CacheMap;

	MaskResult *generate( EditorChunkTerrainPtr terrain );
