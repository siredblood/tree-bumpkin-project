...
void TerrainLodController::cameraPosition( const Vector3& value )
{
	...
	int32 focusX = int32(floorf(cameraPosition_.x / BLOCK_SIZE_METERS));
	int32 focusZ = int32(floorf(cameraPosition_.z / BLOCK_SIZE_METERS));
...
{
	BW_GUARD;
	int32 x = int32(floorf(centre.x / BLOCK_SIZE_METERS));
	int32 z = int32(floorf(centre.z / BLOCK_SIZE_METERS));
...
void BasicTerrainLodController::setCameraPosition(	const Vector3& camPosition )
{
	...
	{
		blocks_[i].second->evaluate( blocks_[i].first, camPosition );
											...
											const Vector3& worldPosition )
{
	...
	blocks_.push_back( std::make_pair( worldPosition, pBlock ) );
}
