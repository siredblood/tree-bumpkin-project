...
MEMTRACKER_DECLARE( Script_Python, "Script - Python", MemTracker::FLAG_DONT_REPORT );
				  ...
				  const char * componentName )
{
	...
	std::string::const_iterator i1 = pythonPaths.begin();
	std::string::const_iterator i2;
	while (true)
	{
		i2 = std::find( i1, pythonPaths.end(), ';' );

		std::string filename( i1, i2 );
...
#define PATHPREFIX "../res/"
	...
	std::string s = pythonPaths;
	std::string commonPath = EntityDef::Constants::commonPath();
	if (!sysPaths.empty()) sysPaths.append( ";" );
	sysPaths = PATHPREFIX+pythonPaths+";"PATHPREFIX +
		commonPath + 
		";"PATHPREFIX + commonPath + "/Lib"
#ifdef _WIN32
		";"PATHPREFIX + commonPath + "/DLLs"
#elif !defined(_LP64)
		";"PATHPREFIX + commonPath + "/lib-dynload"
#else
		";"PATHPREFIX + commonPath + "/lib-dynload64"
#endif
		;
...
#if DELIM != ';'
	std::replace( sysPaths.begin(), sysPaths.end(), ';', DELIM );
#endif
#ifdef _WIN32
	//std::replace( sysPaths.begin(), sysPaths.end(), '/', '\\' );
#endif
	...
	BeginThreadBlockingOperation = &Script::releaseLock;
	CeaseThreadBlockingOperation = &Script::acquireLock;
#endif
...
#ifndef MF_SERVER
	// MaterialKinds is used in py_common.cpp, which is actually only used in the
	// vcproj build, not the Makefile build
	...
	{
		NOTICE_MSG( "Script::init: "
			"Unable to import BWAutoImport. Import again to see errors.\n" );
		PyErr_Print();
	}
...
PyObject * Script::runString( const char * expr, bool printResult )
{
	...
	return PyRun_String( const_cast<char*>( expr ),
...
int PyOS_statType( const char * filename )
{
	...
	//return PyOS_statTypeDefault( filename );
}
	...
	const char * varName )
{
	...
	strcpy(errorString, varName);
	strcat(errorString, " must be set to a bool");
	...
	const char * varName )
{
	...
	{
		rInt = (int)PyLong_AsLong( pObject );
		if (!PyErr_Occurred()) return 0;
		PyErr_Clear();
		rInt = (int)PyLong_AsUnsignedLong( pObject );
		if (!PyErr_Occurred())
		{
			WARNING_MSG( "Assigning value %u (greater than 2147483647) to an "
				"INT32 variable (%s) is deprecated\n", uint32(rInt), varName );
			return 0;
		}
	}
	...
	char errorString[256];
	strcpy(errorString, varName);
	strcat(errorString, " must be set to an int");
	...
	const char * varName )
{
	...
	char errorString[256];
	strcpy(errorString, varName);
	strcat(errorString, " must be set to a long");
	...
	const char * varName )
{
	...
	char errorString[256];
	strcpy(errorString, varName);
	strcat(errorString, " must be set to a unsigned long");
	...
	const char * varName )
{
	if (PyInt_Check( pObject ))
	{
		...
		{
			// PyErr_Format( PyExc_ValueError, "Cannot set %s of type uint to %d",
			//			varName, int(intValue) );
			// return -1;
			WARNING_MSG( "Assigning %d (negative value) to a UINT32 variable "
						"(%s) is deprecated\n", int(intValue), varName );
			rUint = intValue;
			return 0;
		}
	}
	...
	if (PyLong_Check( pObject ))
	{
		rUint = PyLong_AsUnsignedLong( pObject );
		if (!PyErr_Occurred()) return 0;
		PyErr_Clear();
		rUint = (uint)PyLong_AsLong( pObject );
		if (!PyErr_Occurred()) return 0;
	}

	char errorString[256];
	strcpy(errorString, varName);
	strcat(errorString, " must be set to an uint");
	...
	const char * varName )
{
	...
	char errorString[256];
	strcpy(errorString, varName);
	strcat(errorString, " must be set to a float");
	...
	const char * varName )
{
	...
	char errorString[256];
	strcpy(errorString, varName);
	strcat(errorString, " must be a MatrixProvider");
...
int Script::setData( PyObject * pObject, std::string & rString,
	const char * varName )
{
	SmartPointer<PyObject> pStr( PyObject_Str( pObject ), true );

	if (pStr)
	{
		char *ptr_cs;
		Py_ssize_t len_cs;
		PyString_AsStringAndSize(pStr.getObject(), &ptr_cs, &len_cs);
	...
	char errorString[256];
	strcpy( errorString, varName );
	strcat( errorString, " must be set to a string." );
	...
	const char * varName )
{
	...
	if (pUO)
	{
		wchar_t buffer[256];
		int nChars =
			PyUnicode_AsWideChar( reinterpret_cast<PyUnicodeObject*>(pUO.getObject()), buffer, 255 );
		if ( nChars != -1 )
		{
			buffer[nChars]=L'\0';
			rString.assign( buffer );
			return 0;
		}
	}
	...
	char errorString[256];
	strcpy( errorString, varName );
	strcat( errorString, " must be set to a wide string." );
