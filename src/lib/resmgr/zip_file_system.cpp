...
BinaryPtr ZipFileSystem::readFileInternal(const std::string& inPath)
{
	...
	if(!(pCompressed = new char[file.entry_.compressedSize]))
	{
	...
	if(file.entry_.compressionMethod == METHOD_DEFLATE)
	{
		...
		if(!(pUncompressed = new char[file.entry_.uncompressedSize]))
		{
			//ERROR_MSG("ZipFileSystem::readFile Failed to alloc data buffer (%s in %s)\n",
			//	path.c_str(), path_.c_str());
			delete[] pCompressed;
		...
		inflateEnd(&zs);
		pBinaryBlock = 
			new BinaryBlock(pUncompressed, file.entry_.uncompressedSize, "BinaryBlock/ZipRead");
	}
	else
	{
		pBinaryBlock =
			new BinaryBlock(pCompressed, file.entry_.compressedSize,
							"BinaryBlock/ZipRead");
	}
...
void ZipFileSystem::LocalFile::updateHeader( bool clear )
{
	...
	if (!bCompressed_ && pData_)
	{
		...
		if (!ZipFileSystem::zipTest(pData_))
...
{	
	if (pBinary->len() > 2)
	{
		uint16 magic = ((uint16*)pBinary->data())[0];
		if (magic == ZIP_HEADER_MAGIC)
			return true;
	}
	return false;
