...
namespace
{
	...
	// Size of the grid gizmo, in meters (minimum size, can be bigger if neede)
...
)
{
	...
	if ((drawOptions_ & DRAW_ROTATION) != 0)
	{
		intersect = rotationGizmo_->intersects( origin, direction, t );
	}
	if (!intersect && ((drawOptions_ & DRAW_SCALE) != 0))
	{
		intersect = scaleGizmo_->intersects( origin, direction, t );
		if (!intersect)
		{
			intersect = uniformScaleGizmo_->intersects( origin, direction, t );
		}
	}
	return intersect;
}
