		...
		const char * mode )
{
	...
	char* scratchPath = new char[ strlen( fullPath.c_str() ) + 1 ];
	strcpy( scratchPath, fullPath.c_str() );

	char* pathTok = strtok_s(scratchPath, "/\\", &strtokState);
	std::string testPath( pathTok );
	testPath += "/";

	pathTok = strtok_s( NULL, "/\\", &strtokState );
	...
	while ( pathTok )
	{
		...
		{
			HANDLE hFile = FindFirstFile( testPath.c_str(), &fileInfo );
			...
			if ( strcmp( pathTok, fileInfo.cFileName ) )
			{
				WARNING_MSG( "WinFileSystem: Case mismatch opening %s (%s)\n",
					fullPath.c_str(), fileInfo.cFileName );
