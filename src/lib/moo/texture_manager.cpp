	...
	const std::string& allocator)
{
	...
	res = this->find( prepareResourceName(name) );
	if (res) return res;
	...
	const std::string& allocator)
{
	...
	// Try loaing the texture and add it to the cache:
...
bool TextureManager::isTextureFile(const std::string & resourceID) const
{
	...
	// Only recognized extensions are textures:
...
std::string TextureManager::prepareResourceName( const std::string& resourceName )
{
	...
	if (resourceName.rfind("dds") == resourceName.size() - 3)
		return resourceName;
	else if (TextureManager::detailLevel(resourceName)->formatCompressed())
