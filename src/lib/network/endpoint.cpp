...
// Static data
Endpoint * Endpoint::s_pHijackEndpointSync_ = NULL;
Endpoint * Endpoint::s_pHijackEndpointAsync_ = NULL;
BinaryIStream * Endpoint::s_pHijackStream_ = NULL;
Endpoint::FrontEndInterfaces Endpoint::s_frontEndInterfaces_;
...
//	MF_ASSERT( errno == ECONNREFUSED );
	...
	if (ctlHeader != NULL)
	{
		...
		if (errHeader.msg_namelen == 0)
		{
			...
			printf( "Nub::processPendingEvents: "
...
int Endpoint::findDefaultInterface( char * name )
{
	...
	{
		FrontEndInterfaces::iterator iter = s_frontEndInterfaces_.begin();

		while (iter != s_frontEndInterfaces_.end())
		{
			// TODO: This is dodgy. Need a better way to avoid loopback
			// interfaces. The front-end could send up this information.
			if (iter->first != "lo")
			{
...
int Endpoint::findIndicatedInterface( const char * spec, char * name )
{
	...
	if (name[0] == 0)
	{
		...
		if (Endpoint::isHijacked())
		{
			// If this process is acting as a back-end for a front-end process,
			// the network interfaces of the front-end process are considered.
			FrontEndInterfaces::iterator iter = s_frontEndInterfaces_.begin();
			while (iter != s_frontEndInterfaces_.end())
			{
...
/**
 *	This static method sets the hijack stream. When sending from an Endpoint,
 *	data will be sent over this stream instead of the Endpoint's actual socket.
 */
void Endpoint::setHijackEndpoints( Endpoint * pSync, Endpoint * pAsync )
{
	s_pHijackEndpointSync_ = pSync;
	s_pHijackEndpointAsync_ = pAsync;
}


/**
 *	This is a useful helper method that takes care of reading data from a
 *	temporary socket.
 *
 *	@return True if all the desired data was read, otherwise false.
 */
bool Endpoint::hijackRecvAllFrom( void * gramData, int gramSize,
		u_int16_t * pPort, u_int32_t * pAddr )
{
	if (pHijackEndpoint_ == NULL)
	{
		return
	...
	else
	{
		...
		if (!pHijackEndpoint_->recvAll( &msgType, sizeof( msgType ) ) ||
			(msgType != Endpoint::HIJACK_MSG_UDP) ||
			!pHijackEndpoint_->recvAll( &dstFD, sizeof( dstFD ) ) ||
			(dstFD != socket_) ||
			!pHijackEndpoint_->recvAll( &dataLen, sizeof( dataLen ) ) ||
			(dataLen != gramSize + 6) ||
			!pHijackEndpoint_->recvAll( &addr, sizeof( addr ) ) ||
			!pHijackEndpoint_->recvAll( &port, sizeof( port ) ) )
		{
			return false;
		}
		...
		return pHijackEndpoint_->recvAll( gramData, gramSize );
	}
}


/**
 *	This is a useful helper method when creating temporary Endpoints that need
 *	to work when the process is running as a back-end service. It returns the
 *	file descriptor that should be used in the call to select. This is usually
 *	used in conjuction with Endpoint::hijackRecvAllFrom.
 */
int Endpoint::hijackFD() const
{
	return pHijackEndpoint_ ? pHijackEndpoint_->socket_ : socket_;
}


/**
 *	This method registers a network interface of the front-end process with the
 *	back-end process. When this process is a back-end for a front-end process,
 *	the network interfaces of the front-end process are considered instead of
 *	the local ones.
 */
void Endpoint::addFrontEndInterface( const std::string & name, u_int32_t addr )
{
	s_frontEndInterfaces_[ name ] = addr;
}
