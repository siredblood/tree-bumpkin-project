...
//change GUID模型存储
std::map< std::string, EditorChunkModel* > EditorChunkModel::editorGUIDModels_;
...
void EditorChunkModel::addGUIDModel( EditorChunkModel* model, const std::string& guid )
{
	editorGUIDModels_[ guid ] = model;
}

void EditorChunkModel::removeGUIDModel( EditorChunkModel* model )
{
	GUIDModels::iterator iter;
	for ( iter = editorGUIDModels_.begin(); iter != editorGUIDModels_.end(); iter++ )
	{
		EditorChunkModel* tmpModel = iter->second;
		if ( tmpModel == model )
		{
			editorGUIDModels_.erase( iter );
			return;
		}
	}
}
...
bool EditorChunkModel::edShouldDraw()
{
	...
	return (renderScenery == 0) ? false : shouldDraw_;
...
void EditorChunkModel::draw()
{
	...
	if (pSuperModel_ != NULL )
	{
		...
		//if (drawBsp && verts_.size() > 0 && !pPortalMat_ )
		if (drawBsp )//&& !pPortalMat_ )
		{
			//set the transforms
			Matrix transform;
...
bool EditorChunkModel::load( DataSectionPtr pSection, Chunk * pChunk )
{
	...
	if( models.size() )
	{
		...
		//change
		std::string tmpGUID = pOwnSect_->readString( "UniqueID" );
		if ( !tmpGUID.empty() )
			addGUIDModel( this, tmpGUID );
	...
	else
	{
		...
		//change 获取模型的GUID
		guid_ = pOwnSect()->readString("UniqueID",guid_);
		...
		//change static const float lengthLimit = 100.f;
		static const float lengthLimit = 100000.f;
...
bool EditorChunkModel::edSave( DataSectionPtr pSection )
{
	...
	//change 保存GUID
	pSection->writeString("UniqueID",guid_);
...
bool EditorChunkModel::edEdit( ChunkItemEditor & editor )
{
	...
	{
		//change 添加GUID属性
		editor.addProperty(new StaticTextProperty("GUID",new
			ConstantDataProxy<StringProxy>(guid_.toString())));

		//如果为“编辑模式”且是sets目录下的模型，则可进行编辑
		if ( !g_bUserMode && 
			( CommonPublic::instance().containtString( 
				pSuperModel_->topModel(0)->resourceID().c_str(), "sets/" ) ||
			  CommonPublic::instance().containtString( 
				pSuperModel_->topModel(0)->resourceID().c_str(), "sets\\" )))
		{
...
//change 2010-3-5
//设置阴影
void EditorChunkModel::edSetCastsShadow( const bool & b )
{
	castsShadow_ = b;
}
...
//change 2010-3-4
//返回模型GUID
std::string EditorChunkModel::edGUID()
{
...
//change 2010-3-5
//设置模型GUID
void EditorChunkModel::edSetGUID( const std::string & s )
{
	guid_ = UniqueID(s);
	
	if ( !guid_.toString().empty() )
		addGUIDModel( this, guid_ );
}

//change 2010-3-6 
//第一次生成GUID
std::string EditorChunkModel::edCreateGUID()
{
	guid_ = UniqueID::generate();
	return guid_;
}

bool EditorChunkModel::playAnimation(const std::string &newAnimationName)
{
	return setAnimation( newAnimationName );
}
