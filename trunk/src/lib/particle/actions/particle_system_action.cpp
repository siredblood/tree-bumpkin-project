 ...
 *	the rpDerived pointer reference.
 */
int Script::setData( PyObject * pObject, ParticleSystemAction * & rpDerived,
	const char * varName )												
{																		
	BW_GUARD;
	ParticleSystemAction * inputDerived = rpDerived;					
	...
	{																	
		PyErr_Format( PyExc_TypeError, "%s must be set to a "
			"PyParticleSystemAction or None", varName );		
		return -1;														
	}																	
