...
void OrbitorPSA::execute( ParticleSystem &particleSystem, float dTime )
{
	...
	while (current != endOfParticles)
	{
		...
		// Mesh style systems use FixedIndex particles so their indices can 
		// be calculated.
		...
		if (particle.isAlive())
		{
			...
			// a new particle
			if ((indexedOrbits_.find( index ) == indexedOrbits_.end()) || 
					almostEqual( particle.age(), 0 ))
			{
				indexedOrbits_[ index ] = positionOfPS;
			}

			Vector3 orbit = indexedOrbits_[ index ];

			// transform the rotation by the stored PS position that relates 
			// to the particles' age
			Matrix rotation;
			rotation.setRotateY( fmodf( angularVelocity_, MATH_PI * 2.f )  * dTime );
		...
		{
			// Try to use as minimal memory when dealing with FixedIndex
			// not when using contiguous because those particles are always
			// alive
			indexedOrbits_.erase( index );
		}
