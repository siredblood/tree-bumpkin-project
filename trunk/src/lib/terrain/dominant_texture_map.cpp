		...
		image_( "Terrain/DominantTextureMap" )
{
	...
	for (uint32 y=0; y<height; y++)
	{
		...
		{
			MaterialIndex index = (MaterialIndex)0;
			uint8 maxValue = layers[0]->image().getBicubic(
					(float)x * multipliers[0].first,
					(float)y * multipliers[0].second );
	...
	for (uint32 i=0; i<layers.size(); i++)
	{
		textureNames_.push_back( layers[i]->textureName() );
	}
...
MaterialIndex DominantTextureMap::materialIndex( float x, float z ) const
{
	BW_GUARD;
	//this->lock(true);
	x = (x/BLOCK_SIZE_METERS) * (image_.width() - 1) + 0.5f;
	z = (z/BLOCK_SIZE_METERS) * (image_.height()- 1) + 0.5f;
