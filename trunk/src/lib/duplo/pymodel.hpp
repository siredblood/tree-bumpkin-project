 ...
 *  of synchronizing animations with the movement of the model (for example,
...
public:
	...
	const BoundingBox & visibilityBox() const
	...
	PY_AUTO_METHOD_DECLARE_WITH_DOC( RETOWN, getSound,
		ARG( std::string, END ), "Returns a PySound for a 3D sound event at the location of the model." );
	...
	bool attachSound( SoundManager::Event *pEvent );
	SoundManager::EventList &sounds();
...
public:
	...
	PyObject * pyGet_yaw();
	int pySet_yaw( PyObject * value );

	PyObject * pyGet_height();
	int pySet_height( PyObject * value );
	...
	PY_AUTO_METHOD_DECLARE_WITH_DOC( RETOWN, node, ARG( std::string, END ), "Returns the named node." );
...
//	void				removeFromScene();
	...
	void				boundingBoxAcc( BoundingBox & bb, bool skinny );
	void				visibilityBoxAcc( BoundingBox & vbb );
...
private:
	...
	PyModelNode * accessNode( Moo::NodePtr pNode );
	...
	SoundManager::EventList *pSounds_;
	bool				cleanupSounds_;
