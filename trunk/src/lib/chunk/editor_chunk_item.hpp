...
public:
	...
	//2010-3-4 返回模型GUID
	virtual std::string edGUID();

	//2010-3-4 设置模型GUID
	virtual void edSetGUID(const std::string & s){}

	//2010-3-6 第一次生成GUID
	virtual std::string edCreateGUID(){ return ""; }

	//2010-3-5 设置模型阴影
	virtual void edSetCastsShadow(const bool & b){}

	//2010-3-5 设置是否显示模型
	virtual void edSetShouldDraw(const bool & b){}

	//change  
	virtual bool playAnimation( const std::string &newAnimationName ) { return true; }

	//change 
	virtual std::string getParticularPath() { return ""; }
	
	//change 是否绘制表面颜色
	virtual void setDrawSpecialSurface( bool draw ) {}
