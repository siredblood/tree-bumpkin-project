...
/*virtual*/ bool TerrainTextureFunctor::handleKeyEvent(
	...
	Tool &				tool )
{
	if (keyEvent.key() == KeyEvent::KEY_ESCAPE)
	{
		hadEscKey_ = 1;
	}
...
void TerrainTextureFunctor::update( float dTime, Tool & tool )
{
	...
	// quantize badly.  To fix this we multiply the strength at each point
	...
	if (isPainting)
	{
		...
		randomStrength_ = theRandom( randomness_, 1.0f );
	...
	if (!chunkTerrain.brushTextureExist_)
	{
		...
		if (pPaintBrush_->textureMask_)  // has mask, check if tool is visible
		{
			MaskResult * pMaskResult = 
				TextureMaskCache::instance().mask( &chunkTerrain );
		...
		if (needCreate)
		{
			...
			TextureMaskCache::instance().changedLayers( &chunkTerrain );
		}
	...
	Moo::Image< uint8 > &	img ) const
{
	...
	for (uint32 y = 0; y < img.height(); ++y)
	{
		...
		for (uint32 x = 0; x < img.width(); ++x)
		{
			...
			MaskPixel maskPixel = 
				TextureMaskCache::fuzziness( pChunkTerrain, terrainPos, absPos, *pPaintBrush_, maskLayers );
			img.set( x, y, (uint8)255 - maskPixel.fuzziness_ );
		}
	}

	TextureMaskCache::endFuzziness( pChunkTerrain, *pPaintBrush_, maskLayers );
}
			...
			if (adjustedStrength > 0.0f)
			{
				...
				if (adjustedStrength > 0.0f)
				{
					...
					*pPixel          = value;
					...
					{			
						// Normalize the remaining layers not yet 
						// considered.
						uint32 sumOther = 0;
						for (size_t i = 0; i < level; ++i)
						{
		...
		for (int32 x = minx; x <= maxx; ++x)
		{
			...
			if (adjustedStrength > 0.0f)
			{
				...
				if (adjustedStrength > 0.0f)
				{
					...
					adjustedStrength *= theRandom( randomness_, 1.0f );
