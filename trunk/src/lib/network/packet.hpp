...
private:
	...
	/// Pool from which instances of Packet are drawn.
...
private:
	...
	/// in Nub::send() whilst footers are being written, and when receiving, it
	/// is incorrect until processOrderedPacket() strips the fragment footers.
	int			msgEndOffset_;
...
public:
	...
	{
		return s_allocator_.allocate( size );
	}

	static void operator delete( void * pInstance )
	{
		s_allocator_.deallocate( pInstance );
	}
#endif
	...
	SeqNum & fragBegin() { return fragBegin_; }

	SeqNum fragEnd() const { return fragEnd_; }
	SeqNum & fragEnd() { return fragEnd_; }

	Checksum & checksum() { return checksum_; }
	 ...
	 *  be called from Nub::send() and assumes that size_ has been artificially
	 *  increased so that it points to the end of the footers, the idea being
	 *  that we work back towards the real body end.
	 */
	template <class TYPE>
