	...
	bool init( const char * path, PyObject * function, PyObject * argList )
	{
		...
		for (Py_ssize_t i = 0; i < seqSize; i++)
		{
			...
			if (pArg == NULL)
			{
				...
				//	"element %zd from argList sequence.\n", i );
			...
			else if (PyTuple_Check( pArg ) && (PyTuple_Size( pArg ) == 2))
			{
				...
				{
					PyErr_Format( PyExc_RuntimeError,
							"Failed to decode argument tuple for element %zd "
				...
				if (!PyString_Check( pName ) || !PyType_Check( pType ))
				{
					PyErr_Format( PyExc_ValueError,
							"Invalid argument tuple types for element %zd "
							"of arg list sequence.", i );
					return false;
			...
			{
				PyErr_Format( PyExc_RuntimeError,
						"Invalid type at element %zd of arg list sequence.", i );
				return false;
			}
