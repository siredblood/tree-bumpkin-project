...
std::string WaitDlg::lastString_ = "";
...
ISplashVisibilityControl* WaitDlg::getSVC()
{
	if ( !s_dlg_ )
	{
		...
		{
			const CWnd* alwaysOnTop = (!IsDebuggerPresent()) ? &CWnd::wndTopMost : 0;
...
void WaitDlg::overwriteTemp( const std::string& info, int delay )
{
	...
	char lastC_String[256];
	s_dlg_->GetDlgItem( IDC_WAITDLG_INFO )->GetWindowText( lastC_String, 255);
	lastString_ = std::string(lastC_String);
	s_dlg_->GetDlgItem( IDC_WAITDLG_INFO )->SetWindowText( info.c_str() );
	s_dlg_->ShowWindow( SW_SHOW );
	s_dlg_->UpdateWindow();
}
