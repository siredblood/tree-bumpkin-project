...
ChunkItemFactory::Result ChunkParticles::create( Chunk * pChunk, DataSectionPtr pSection )
{
	...
	if (!pParticles->load( pSection ))
	{
		...
		std::string err = L( "PARTICLE/CHUNK_PARTICLES/FAILED_LOAD" ); 
		if ( pSection )
		{
...
{
	bb.addYBounds( localTransform_.applyToOrigin().y );
	return true;
}
