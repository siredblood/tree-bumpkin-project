...
/*virtual*/ bool ChunkLink::isValid(std::string &failureMsg) const
{
	...
	if (startItem_ == NULL)
    {
        failureMsg = L("CHUNK/FRAMEWORK/CHUNK_LINK/NO_START_ITEM");
        return false;
    }

    if (endItem_ == NULL)
    {
        failureMsg = L("CHUNK/FRAMEWORK/CHUNK_LINK/NO_END_ITEM");
        return false;
    }

    if (direction_ == DIR_NONE)
    {
        failureMsg = L("CHUNK/FRAMEWORK/CHUNK_LINK/DIRECTION_IS_NONE");
        return false;
    }
    ...
    )
    {
        ...
        {
            failureMsg = L("CHUNK/FRAMEWORK/CHUNK_LINK/NODES_IN_DIFFERENT_GRAPH");
            return false;
        }
    }
    ...
    if (chunk() == NULL)
    {
        failureMsg = L("CHUNK/FRAMEWORK/CHUNK_LINK/LINK_NOT_IN_CHUNK");
        return false;
    }
