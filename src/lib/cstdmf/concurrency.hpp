...
/**
 *	This class is a recursion-checking mutex
 *
 *	If grab() returns false, you do not own the mutex, but your thread does
 */
class RecursionCheckingMutex
{
public:
	RecursionCheckingMutex() : gone_( false ) { InitializeCriticalSection( &mutex_ ); }
	~RecursionCheckingMutex()	{ DeleteCriticalSection( &mutex_ ); }
...
public:
	RecursionCheckingMutex()
	{
		...
		pthread_mutex_init( &mutex_, &errorchecking );
...
/**
 *	No-op version of RecursionCheckingMutex
 */
class RecursionCheckingMutex
{
public:
	RecursionCheckingMutex(): locked_( false ) {};
