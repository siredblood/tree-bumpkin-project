					...
					std::string *error/* = NULL*/)
{
	...
	if ( !CommonTerrainBlock2::internalLoad( filename, pTerrain, worldPosition, 
	...
	evaluate( worldPosition, cameraPosition );
	stream();
	...
	BasicTerrainLodController::instance().addBlock( this, worldPosition );
...
void TerrainBlock2::createUMBRAMesh( UMBRAMesh& umbraMesh ) const
{
	...
	float step = BLOCK_SIZE_METERS / (MESH_SIZE);
 ...
 *
 *	@param blockWorldPos	The world position of the block.
 *	@param cameraWorldPos	The world position of the camera.
 */
void TerrainBlock2::evaluate(	const Vector3& blockWorldPos, 
								const Vector3& cameraWorldPos )
{
	...
	//
	{
		...
		distanceInfo_.relativeCameraPos_ = cameraWorldPos - blockWorldPos;
...
BoundingBox const & TerrainBlock2::boundingBox() const
{
	...
	bb_.setBounds( Vector3( 0.f, minh, 0.f ), 
		Vector3( BLOCK_SIZE_METERS, maxh, BLOCK_SIZE_METERS ) );
