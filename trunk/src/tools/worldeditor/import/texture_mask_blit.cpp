...
public:
	...
	bool		inMemory() const;
	void		inMemory( bool inMem );

	bool		forceSave() const;
	void		forceSave( bool force );

	EditorChunkTerrain * getChunkTerrain( 
				const std::string & chunkName,
				bool forceSave );

	Terrain::EditorBaseTerrainBlock *getTerrainBlock(
				const std::string & chunkName,
				bool forceSave );	
...
/**
 *	This gets whether the chunk was already in memory.
 *
 *	@return				True if the chunk was already in memory, false if it
 *						was on disk.
 */
bool TerrainBlockInfo::inMemory() const
{
	return inMemory_;
}
...
/**
 *	This sets whether the chunk was already in memory.
 *
 *	@param inMem		If true then the chunk is marked as already being in
 *						memory, if false then the chunk was loaded from disk.
 */
void TerrainBlockInfo::inMemory( bool inMem )
{
 	inMemory_ = inMem;
}
 ...
 *
 *	@return				True if the chunk should be saved and ejected, false 
...
/**
 *	This function gets the ChunkTerrain for the given block.
 *
 *  @param chunkName	The name of the chunk to load.
 *  @param forceSave	Force the chunks to memory and save. 
 *	@return				The chunk terrain for the given chunk.
 */
EditorChunkTerrain *
TerrainBlockInfo::getChunkTerrain(	
	const std::string &	chunkName,		
	bool				forceSave )
{
	// Get the chunk and if necessary load it:
	Chunk *pChunk = 
	...
	return static_cast< EditorChunkTerrain * >( pChunkTerrain );
}
 ...
 */
Terrain::EditorBaseTerrainBlock * TerrainBlockInfo::getTerrainBlock(	
	const std::string &	chunkName,		
	bool				forceSave )
{
	// Get the terrain:
	EditorChunkTerrain *chunkTerrain = 
		this->getChunkTerrain( chunkName, forceSave );
	...
	{
		// Finally, we can get the EditorBaseTerrainBlock.
		return 
	...
	Terrain::EditorBaseTerrainBlock	*	pTerrainBlock )
{
	...
	if (forceSave_)
	{
		ok = ok && pTerrainBlock->rebuildLodTexture( pChunk_->transform() );
		...
		ok = ok && pTerrainBlock->save( filename );
...
/*virtual*/ bool ImportVisitor::visit(
	...
	uint32				polesHigh )
{
	Terrain::EditorBaseTerrainBlock *pTerrainBlock	= NULL;
	EditorChunkTerrain				*pChunkTerrain	= NULL;
	TerrainBlockInfo				info;

	pChunkTerrain = info.getChunkTerrain( chunkName, forceSave_ );
	if (pChunkTerrain == NULL)
	{
		return false;
	}
	...
	MaskResult const *pMaskResult = 
		TextureMaskCache::instance().mask( pChunkTerrain );
	...
	for (int dy = dbottom; dy <= dtop; ++dy)
	{
		...
		for (int dx = dleft; dx <= dright; ++dx)
		{
			...
			*pLayers[ layer ] = value;				
			...
			// Normalize all of the other layers:
	...
	return info.setTerrainBlock( pTerrainBlock );
};
	...
	ChunkVisitor		&visitor )
{
	...
	// Convert the selection from meters into the giant bitmap coordinates:
