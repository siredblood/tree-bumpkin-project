...
{
	MF_ASSERT( s_currentInstance_ == NULL );
...
void HeightModule::render(float dTime)
{
	...
	if( HeightMap::instance().aborted() )
	{
		if( !abortedText_.exists() )
		{
			abortedText_ = new TextGUIComponent( TextGUIComponent::s_defFont() );
...
bool HeightModule::handleKeyEvent(const KeyEvent & keyEvent)
{
	...
	if (keyEvent.key() == KeyEvent::KEY_1 && keyEvent.isKeyDown() && !keyEvent.modifiers())
	{
		PanelManager::instance().setToolMode( "Objects" );
	}
	if (keyEvent.key() == KeyEvent::KEY_2 && keyEvent.isKeyDown() && !keyEvent.modifiers())
	{
		PanelManager::instance().setToolMode( "TerrainTexture" );
	}
	if (keyEvent.key() == KeyEvent::KEY_3 && keyEvent.isKeyDown() && !keyEvent.modifiers())
	{
		PanelManager::instance().setToolMode( "TerrainHeight" );
	}
	if (keyEvent.key() == KeyEvent::KEY_4 && keyEvent.isKeyDown() && !keyEvent.modifiers())
	{
		PanelManager::instance().setToolMode( "TerrainFilter" );
	}
	if (keyEvent.key() == KeyEvent::KEY_5 && keyEvent.isKeyDown() && !keyEvent.modifiers())
	{
		PanelManager::instance().setToolMode( "TerrainMesh" );
	}
	if (keyEvent.key() == KeyEvent::KEY_7 && keyEvent.isKeyDown() && !keyEvent.modifiers())
	{
		PanelManager::instance().setToolMode( "Project" );
	}
        ...
        if (doUndo)
        {
            result =
                ...
                (
                    terrainTopLeft_    .x, terrainTopLeft_    .y,
                    terrainBottomRight_.x, terrainBottomRight_.y,
                    L("WORLDEDITOR/WORLDEDITOR/HEIGHT/HEIGHT_MODULE/IMPORT_TERRAIN"),
                    showProgress
                );
        }
        if (result)
...
void HeightModule::writeStatus()
{
	...
	//Panel 0 - memory load
	WorldManager::instance().setStatusMessage( 1,
		L("WORLDEDITOR/WORLDEDITOR/HEIGHT/HEIGHT_MODULE/MEMORY_LOAD",
	...
	//		L("WORLDEDITOR/WORLDEDITOR/HEIGHT/HEIGHT_MODULE/CHUNK_LOADED_WITH_DIRTY",
	//		EditorChunkCache::chunks_.size(), dirtyTotal, numLodTex ) );
	//}
	//else
	//{
	//	WorldManager::instance().setStatusMessage(5,
	//		L("WORLDEDITOR/WORLDEDITOR/HEIGHT/HEIGHT_MODULE/CHUNK_LOADED",
