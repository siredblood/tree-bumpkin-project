 ...
 */
void MeshGUIAdaptor::draw( bool overlay )
{
	if ( visible() )
	{
		Moo::rc().push();
		...
		if ( !momentarilyInvisible_ )	//don't check the texture_ member ( see SGC.ipp )
		{
...
void MeshGUIAdaptor::addAsSortedDrawItem()
{
	if ( visible() )
	{
		//Add children to sorted tree
		SimpleGUIComponent::addAsSortedDrawItem();
