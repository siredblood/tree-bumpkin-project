...
void TintShaderPSA::execute( ParticleSystem &particleSystem, float dTime )
{
	...
	if ( modulator_ )
	{
		modulator_->tick( dTime );
		modulator_->output( multiplier );
	}

#ifdef FAST_MATH
	Vector4Fast multiplierFast( multiplier );
#endif
...
#ifdef FAST_MATH
			Vector4Fast upperColour( upper->second );
			Vector4Fast lowerColour( lower->second );

			Vector4Fast colour( upperColour - lowerColour );

			colour *= ( effectiveAge - lower->first ) /
				( upper->first - lower->first);

			colour += lowerColour;
			colour *= multiplierFast;

			colour.saturate();
			colour *= 255.f;

			int intColour[4];
			colour.castToInt( intColour );

			particle.colour( Colour::getUint32NoClamp( intColour ) );
