...
void CursorCamera::set( const Matrix & viewMatrix )
{
	...
	pitch_ = atan2f( cameraDirInWS_.y,
		sqrtf( cameraDirInWS_.z * cameraDirInWS_.z +
			cameraDirInWS_.x * cameraDirInWS_.x ) );
	yaw_ = atan2f( cameraDirInWS_.x, cameraDirInWS_.z );
...
void CursorCamera::update( float deltaTime )
{
	...
	if ( pTarget_ && pSource_ )
	{
		...
		*/
		...
		{
			inaccuracyProvider_->tick( deltaTime );
