	...
	virtual void save( DataSectionPtr pVisualSection, DataSectionPtr pPrimitives )
	{
		...
		if (pRenderSet_ && pPrimitives_)
		{
			...
			for (uint i = 0; i  < geometrySections.size(); i++)
			{
                ...
                pPrimitives->writeBinary( verts, new BinaryBlock( pVerts->data(), pVerts->len(), "BinaryBlock/VisualSplitter" ) );
...
public:
	...
	public:
		...
		virtual uint index( uint32 vertex, uint32 index ) const
		{
			...
			const VertexType& vert = verts_[vertex];
			if (index == 0)
				return vert.index_ / 3;
			if (index == 1)
				return vert.index2_ / 3;
			return vert.index3_ / 3;
		}
		...
		virtual VertexHolder* copyRemapped( const std::vector<uint16>& mappings ) const
		{
			...
			VertexHolder* pOut = new MultiBlendHolder<VertexType>( &out.front(), out.size(),
				formatName_ );
			return pOut;
		}
		...
		{
			BW_GUARD;
			return VertexHolder::createVertexSection( verts_, formatName_ );
		}
	 ...
	 */
	template< typename VertexType, typename IndexType = float >
	...
	public:
		...
		virtual VertexHolder* copyRemapped( const std::vector<uint16>& mappings ) const
		{
			...
			std::vector< VertexType > out;
			remap( out, verts_, mappings );
			VertexHolder* pOut = new SingleBlendHolder<VertexType>( &out.front(), out.size(),
				formatName_ );
			return pOut;
		}
	...
	public:
		...
		NewRenderSet( const std::vector<uint16>& indices, SmartPointer<VertexHolder> pVertices,
				...
				const std::vector<std::string>& nodes, const std::string& vertsName,
	...
	virtual bool compute()
	{
		...
		for (uint i = 0; i  < geometrySections.size(); i++)
		{
			...
			SmartPointer<VertexHolder> pVHolder = readVertices( pVerts );
				...
				while (boneRelations.size())
				{
					...
					if (boneRelations.createList( nodeLimit_, boneList ))
					{
						...
						new NewRenderSet( splitIndices, pVHolder, pMTHolderForRS, boneList, nodes_,
							verts, prim, j, splitIndex, pMaterialSection ) );
...
private:
	 ...
	 */
	VertexHolder* readVertices( BinaryPtr pVerts )
	{
		BW_GUARD;	
#define CREATE_HOLDER( name, format, container ) \
	if (std::string(pVh->vertexFormat_) == #name)\
    {\
		pHolder = new container<format>( \
		reinterpret_cast< const format* >( pVh + 1 ), pVh->nVertices_, #name );\
    }
		const VertexHeader* pVh = reinterpret_cast<const VertexHeader*>( pVerts->data());
		VertexHolder* pHolder = NULL;
		CREATE_HOLDER( xyznuvi, VertexXYZNUVI, SingleBlendHolder )
		else CREATE_HOLDER( xyznuvitb, VertexXYZNUVITB, SingleBlendHolder )
		else CREATE_HOLDER( xyznuviiiww, VertexXYZNUVIIIWW, MultiBlendHolder )
		else CREATE_HOLDER( xyznuviiiwwtb, VertexXYZNUVIIIWWTB, MultiBlendHolder )
		return pHolder;
	}
