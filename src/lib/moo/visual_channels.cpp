...
void VisualDrawItem::init( VertexSnapshotPtr pVSS, PrimitivePtr pPrimitives, 
		...
		float distance, Moo::VertexBuffer pStaticVertexColours, 
		bool sortInternal )
{
	BW_GUARD;
	pVSS_ = pVSS;
...
void VisualDrawItem::drawSorted()
{
	...
	// Check the indices are still valid
	if (indexSlot_==NULL || !indexSlot_->valid())
		pVSS_->resetUsage();
	...
	uint32 vertexOffset = pVSS_->setVertices( pg.startVertex_, 
		pg.nVertices_, pStaticVertexColours_.valid() );
	...
	if (pStaticVertexColours_.valid())
		pStaticVertexColours_.set( 1, 0, sizeof( DWORD ) );
	...
	if (maxIndex < Moo::rc().maxVertexIndex()) 
 	{ 
		...
		{
			uint32 firstIndex = indexSlot_==NULL ? dib.lockIndex() : 
													indexSlot_->offset();
...
void VisualDrawItem::drawUnsorted( )
{
	...
	if (vertexOffset == pg.startVertex_)
	{
		...
		else
		{
			...
			// Reset stream 1 if static vertex colours are being used
			if (pStaticVertexColours_.valid())
				Moo::VertexBuffer().set( 1, 0, sizeof( DWORD ) );
		}
	}
	else
	{
...
void ShimmerChannel::addItem( VertexSnapshotPtr pVSS, PrimitivePtr pPrimitives, 
		...
		Moo::VertexBuffer pStaticVertexColours )
{
	BW_GUARD;
	if (enabled_)
	{
