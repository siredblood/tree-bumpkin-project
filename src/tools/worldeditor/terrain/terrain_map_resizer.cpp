...
{
	Vector3 pos( GRID_RESOLUTION * x, 0.0f, GRID_RESOLUTION * z );
	...
	if (!pBlock->load( terrainName, pos, pos, pError ))
	{
		if (pError)
		{
	...
	ProgressTask * pProgress /* = NULL */ )
{
	...
	for (int32 z = minZ; z <= maxZ; z++)
	{
		for (int32 x = minX; x <= maxX; x++)
		{
			...
			CopyFile( original.c_str(), converted.c_str(), FALSE );
	...
	for (int32 z = minZ; z <= maxZ; z++)
	{
		for (int32 x = minX; x <= maxX; x++)
		{
			...
			MoveFile( converted.c_str(), original.c_str() );
			if (pProgress)
			{
				pProgress->step();
			}
