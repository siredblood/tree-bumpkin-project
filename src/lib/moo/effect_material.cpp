...
void EffectMaterial::replaceDefaults()
{
	...
	if ( pManagedEffect_ )
	{
		...
		while (it != end)
		{
			...
			if ( this->isDefault( pProp ) )
			{
	            pProp->apply( pManagedEffect_->pEffect(),propertyHandle );
				D3DXPARAMETER_DESC desc;
				pManagedEffect_->pEffect()->GetParameterDesc( propertyHandle, &desc );
				...
				while ( fit != fend )
				{
					EffectPropertyFunctorPtr f = fit->second;
					
					if ( f->check( &desc ) )
					{
						pProp = f->create(propertyHandle, pManagedEffect_->pEffect());
						break;
					}
					else	
					{
