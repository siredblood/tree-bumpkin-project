...
const std::string PageChunkTexture::contentID = "PageChunkTexture";
...
{
	chunkID_.SetWindowText( chunk.c_str() );
	if ( chunk.empty() )
	{
	...
	Chunk* pChunk =
		ChunkManager::instance().findChunkByName( chunk, WorldManager::instance().chunkDirMapping() );
	if ( !pChunk )
	{
		//ERROR_MSG( "PageChunkTexture::chunk: Invalid chunk name %s.\n", chunk.c_str() );
		chunkID_.SetWindowText( "" );
		PageChunkTextureView::chunk( NULL );
		setTextureControl( NULL );
		return;
	}
...
void PageChunkTexture::deleteLayers( std::vector<size_t> const &selection )
{
	...
	Chunk* pChunk =
		ChunkManager::instance().findChunkByName(
			chunkName(), WorldManager::instance().chunkDirMapping() );
	EditorChunkTerrainPtr ect = texturesControl_.terrain();
	if ( !block || !pChunk || !ect)
		return;

	UndoRedo::instance().add( new TerrainTextureLayerUndo( block, pChunk ) );
	UndoRedo::instance().barrier( L("WORLDEDITOR/GUI/PAGE_CHUNK_TEXTURE/LAYER_DELETE"), true );
...
void PageChunkTexture::mergeLayers( std::vector<size_t> const &selection )
{
	...
	UndoRedo::instance().add( new TerrainTextureLayerUndo( block, pChunk ) );
	UndoRedo::instance().barrier( L("WORLDEDITOR/GUI/PAGE_CHUNK_TEXTURE/LAYERS_MERGED"), true );
...
LRESULT PageChunkTexture::OnActivateTool( WPARAM wParam, LPARAM lParam )
{
	...
	const char *toolId = (const char *)wParam;
...
LRESULT PageChunkTexture::OnTerrainTextureRightClick(WPARAM wParam, LPARAM lParam)
{
	...
	menu.addItem( L("WORLDEDITOR/GUI/PAGE_CHUNK_TEXTURE/MENU_USE_TEXTURE"), selectCmd );

	Terrain::EditorBaseTerrainBlock* block = texturesControl_.terrainBlock();
	if ( block && block->textureLayer( (int)lParam ).hasUVProjections() )
	{
		menu.addItem( L("WORLDEDITOR/GUI/PAGE_CHUNK_TEXTURE/MENU_EDIT_PROJECTION_AND_SCALE"), editCmd );
	}

	if (block && block->numberTextureLayers() != 0)
	{
		menu.addItem( L("WORLDEDITOR/GUI/PAGE_CHUNK_TEXTURE/MENU_SET_TEXTURE_MASK"), maskCmd );
	}
	...
	if ( block && selection.size() > 0 && selection.size() != block->numberTextureLayers())
	{
		...
		if (selection.size() == 1)
			menu.addItem( L("WORLDEDITOR/GUI/PAGE_CHUNK_TEXTURE/MENU_REMOVE_TEXTURE"), deleteCmd );
		else
			menu.addItem( L("WORLDEDITOR/GUI/PAGE_CHUNK_TEXTURE/MENU_REMOVE_TEXTURES"), deleteCmd );
	}
	...
	if (block && selection.size() > 1)
	{
		...
		menu.addItem( L("WORLDEDITOR/GUI/PAGE_CHUNK_TEXTURE/MENU_MERGE_TEXTURES"), mergeCmd );
	}
...
void PageChunkTexture::OnBnClickedShow()
{
	...
	if ( !chunk.empty() )
	{
		pChunk = ChunkManager::instance().findChunkByName( chunk, WorldManager::instance().chunkDirMapping() );
	}
