...
/**
 *	SmartList: Inherits from CListCtrl to implement a virtual list optimized
...
public:
	...
	virtual void listItemToolTip( int index, std::string& info ) = 0;
};


// SmartListCtrl data provider base abstract class
...
private:
	...
	void getData( int index, std::string& text, int& image, bool textOnly = false );
