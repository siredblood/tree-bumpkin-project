...
{
	is >> version_ >> loggerID_ >> uid_ >> pid_ >> componentName_;
}

// This is here sot that the watcher can find Mercury::watcherValueToString
	...
	endpoint_( endpoint )
{
	...
	TRACE_MSG( "Total # loggers on network: %zu\n", loggers_.size() );
...
LoggerMessageForwarder::~LoggerMessageForwarder()
{
	...
	if (spamTimerID_ != TIMER_ID_NONE)
	{
		nub_.cancelTimer( spamTimerID_ );
	}
}
...
void LoggerMessageForwarder::init()
{
	MF_WATCH( "logger/add", *this,
			...
			"Used by bwlogger to add itself as a logging destination" );
	MF_WATCH( "logger/del", *this,
			&LoggerMessageForwarder::watcherHack,
			&LoggerMessageForwarder::watcherDelLogger,
			"Used by bwlogger to remove itself as a logging destination" );
	...
	MF_WATCH( "debug/hasDevelopmentAssertions", DebugFilter::instance(),
			MF_ACCESSORS( bool, DebugFilter, hasDevelopmentAssertions ),
	   "If true, the process will be stopped when a development-time "
		   "assertion fails" );

	// Register a timer with the Nub for doing spam suppression
	spamTimerID_ = nub_.registerTimer( 1000000 /* 1s */, this );
}
...
void SimpleLoggerMessageForwarder::addLogger( const Mercury::Address & addr )
{
	...
	//INFO_MSG( "LoggerMessageForwarder::addLogger: Added %s. # loggers = %zu\n",
	//		(char *)addr, loggers_.size() );
 ...
 */
int LoggerMessageForwarder::handleTimeout( TimerID id, void * arg )
{
