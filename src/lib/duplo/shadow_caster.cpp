 ...
 */
bool ShadowCaster::init( ShadowCasterCommon* pCommon, bool halfRes, Moo::RenderTarget* pDepthParent )
{
	BW_GUARD;
	bool success = false;
	if (Moo::rc().supportsTextureFormat( D3DFMT_G16R16F ))
	{
		...
		pShadowTarget_ = new Moo::RenderTarget( "ShadowTarget" );
...
void ShadowCaster::begin( const BoundingBox& worldSpaceBB, const Vector3& lightDirection )
{
	...
	if ( pShadowTarget_->valid() && pShadowTarget_->push() )
	{
