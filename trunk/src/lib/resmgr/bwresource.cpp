...
public:
	...
	static const std::string	getAppDataDirectory( const std::string & company, const std::string & product );

	// Version info (VS_VERSION_INFO) access methods
	const std::string			getAppVersionString( const std::string & stringName );
...
/*static*/ const std::string BWResourceImpl::getAppDataDirectory(
	const std::string & company, const std::string & product )
{
	char appDataPath[MAX_PATH];
	HRESULT result;
	// Much of this could be replaced with SHGetFolderPathAndSubDir, but
	// that is only support from Windows XP onwards
	result = SHGetFolderPath( NULL, CSIDL_APPDATA|CSIDL_FLAG_CREATE, NULL,
		SHGFP_TYPE_CURRENT, appDataPath );
	if( SUCCEEDED( result ) )
	{
		if ( company != "" && company.length() < MAX_PATH )
		{
			PathAppend( appDataPath, company.c_str() );
		}
		if ( product != ""  && product.length() < MAX_PATH )
		{
			PathAppend( appDataPath, product.c_str() );
		}
		if( !PathFileExists( appDataPath ) )
		{
			int error = SHCreateDirectoryEx( NULL, appDataPath, NULL );
			if( error != ERROR_SUCCESS )
			{
				WARNING_MSG( "BWResourceImpl::getAppDataDirectory: Couldn't "
					"create application data directory '%s:' 0x%08x\n",
					appDataPath, error );
				return "";
			}
		}
		std::string appDataString( appDataPath );
		std::replace( appDataString.begin(), appDataString.end(), '\\', '/' );
		appDataString += "/";
	...
	const std::string & stringName )
{
	...
	// Blah. VerQueryValue on MSDN claims second paramter is LPCSTR,
	// but Visual Studio 2005 (SP1) claims it's an LPSTR.
	// So we can't trust it not to screw with the supplied buffer.

	char* scratchBuf = new char[ lookupPath.str().length() + 1 ];
	lookupPath.str().copy( scratchBuf, lookupPath.str().length() );
	scratchBuf[ lookupPath.str().length() ] = '\0';

	unsigned int resultLen;
	char* resultPtr;
	if ( VerQueryValue( appVersionData_, scratchBuf,
...
const std::string BWResourceImpl::getAppVersionLanguage()
{
	...
	if ( VerQueryValue( appVersionData_, "\\VarFileInfo\\Translation",
...
void BWResourceImpl::getAppVersionData()
{
	...
	int result =
		GetModuleFileName( NULL, fileNameBuffer, sizeof( fileNameBuffer ) );
	if ( result == 0 )
	{
	...
	int versionSize;
	DWORD scratchValue;
	versionSize = GetFileVersionInfoSize( fileNameBuffer, &scratchValue );
	if ( versionSize == 0 )
	{
	...
	if ( ! GetFileVersionInfo( fileNameBuffer, NULL, versionSize,
		appVersionData_ ) )
	{
		WARNING_MSG( "BWResourceImpl::getAppVersionData: "
...
{
	std::string buf = path;
	char* tok = strtok( const_cast<char*>( buf.c_str() ), BW_RES_PATH_SEPARATOR );
	...
	while (tok != 0)
	{
		...
		if (last > tok)
		{
			std::string tmpTok  = tok;
			std::string path;
			std::string::size_type pos = tmpTok.find_first_of( "\\" );
			
			if ( pos != std::string::npos )
				path = tok;
			else
				path = BWResourceImpl::formatSearchPath( tok, pathXml );
		...
		tok = strtok( 0, BW_RES_PATH_SEPARATOR );
	}
 ...
 *
 *	@param company	The name of the company to use in the path
 *
 *	@param product	The name of the product to use in the path
 *
 *	@return	The path to the existant directory, terminated with a slash, or "" if the directory
 *			could not be found or created.
 *
 */
/*static*/ const std::string BWResource::appDataDirectory( const std::string & company, const std::string & product )
{
	return BWResourceImpl::getAppDataDirectory( company, product );
}
