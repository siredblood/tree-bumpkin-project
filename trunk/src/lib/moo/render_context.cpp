...
uint32 s_numPreloadResources = 0;
	...
	static const size_t MAX_PRELOAD_COUNT = 2000;
#endif
} // namespace anonymous
...
D3DFORMAT RenderContext::getMatchingZBufferFormat( D3DFORMAT colourFormat, bool stencilWanted, bool & stencilAvailable )
{
	...
	if( colourFormat == D3DFMT_R8G8B8 || colourFormat == D3DFMT_X8R8G8B8 || colourFormat == D3DFMT_A8R8G8B8 )
	{
		if ( !stencilWanted )
		{
			if( SUCCEEDED( d3d_->CheckDeviceFormat( adapter, deviceType_, colourFormat, D3DUSAGE_DEPTHSTENCIL, D3DRTYPE_SURFACE, D3DFMT_D32 ) ) &&
				SUCCEEDED( d3d_->CheckDepthStencilMatch( adapter, deviceType_, colourFormat, colourFormat, D3DFMT_D32 ) ) )
				return D3DFMT_D32;
			if( SUCCEEDED( d3d_->CheckDeviceFormat( adapter, deviceType_, colourFormat, D3DUSAGE_DEPTHSTENCIL, D3DRTYPE_SURFACE, D3DFMT_D24X8 ) ) &&
				SUCCEEDED( d3d_->CheckDepthStencilMatch( adapter, deviceType_, colourFormat, colourFormat, D3DFMT_D24X8 ) ) )
				return D3DFMT_D24X8;
			if( SUCCEEDED( d3d_->CheckDeviceFormat( adapter, deviceType_, colourFormat, D3DUSAGE_DEPTHSTENCIL, D3DRTYPE_SURFACE, D3DFMT_D24S8 ) ) &&
				SUCCEEDED( d3d_->CheckDepthStencilMatch( adapter, deviceType_, colourFormat, colourFormat, D3DFMT_D24S8 ) ) )
				return D3DFMT_D24S8;
			if( SUCCEEDED( d3d_->CheckDeviceFormat( adapter, deviceType_, colourFormat, D3DUSAGE_DEPTHSTENCIL, D3DRTYPE_SURFACE, D3DFMT_D24X4S4 ) ) &&
				SUCCEEDED( d3d_->CheckDepthStencilMatch( adapter, deviceType_, colourFormat, colourFormat, D3DFMT_D24X4S4 ) ) )
	...
	if ( !stencilWanted )
	{
		if( SUCCEEDED( d3d_->CheckDeviceFormat( adapter, deviceType_, colourFormat, D3DUSAGE_DEPTHSTENCIL, D3DRTYPE_SURFACE, D3DFMT_D16 ) ) &&
			SUCCEEDED( d3d_->CheckDepthStencilMatch( adapter, deviceType_, colourFormat, colourFormat, D3DFMT_D16 ) ) )
			return D3DFMT_D16;
		if( SUCCEEDED( d3d_->CheckDeviceFormat( adapter, deviceType_, colourFormat, D3DUSAGE_DEPTHSTENCIL, D3DRTYPE_SURFACE, D3DFMT_D15S1 ) ) &&
			SUCCEEDED( d3d_->CheckDepthStencilMatch( adapter, deviceType_, colourFormat, colourFormat, D3DFMT_D15S1 ) ) )
			return D3DFMT_D15S1;
		if( SUCCEEDED( d3d_->CheckDeviceFormat( adapter, deviceType_, colourFormat, D3DUSAGE_DEPTHSTENCIL, D3DRTYPE_SURFACE, D3DFMT_D24X8 ) ) &&
			SUCCEEDED( d3d_->CheckDepthStencilMatch( adapter, deviceType_, colourFormat, colourFormat, D3DFMT_D24X8 ) ) )
			return D3DFMT_D24X8;
...
void RenderContext::initRenderStates()
{
	...
	// Turn on clipping for fixed function cards.
...
const std::string RenderContext::screenShot( const std::string& fileExt /*= "bmp"*/, const std::string& fileName /*= "shot"*/, bool autoNumber /*= true*/ )
{
	...
	{
		if (SUCCEEDED( D3DXSaveSurfaceToFile( name.c_str(), format[ext], backBuffer.pComObject(), NULL, NULL) ) )
		{
...
bool RenderContext::RenderTargetStack::push( RenderContext* rc )
{
	...
	si.renderSurfaces_.push_back(ts);
	ts = NULL;

	ts = rc->getRenderTarget( 1 );
	if( ts )
	{
		si.renderSurfaces_.push_back(ts);
		ts = NULL;
...
bool RenderContext::RenderTargetStack::pop( RenderContext* rc )
{
	...
	if (SUCCEEDED(hr))
	{
		if ( si.renderSurfaces_.size()>1 )
			hr = rc->setRenderTarget( 1, si.renderSurfaces_[1] );
		else
...
void RenderContext::preloadDeviceResources( uint32 timeLimitMs )
{
		...
		preloadResourceMutex_.grabTry())
	{
		...
		{
			preloadResourceList_.front()->PreLoad();
...
void RenderContext::addPreloadResource(IDirect3DResource9* preloadResource)
{
	...
	SimpleMutexHolder smh( preloadResourceMutex_ );
	#ifdef EDITOR_ENABLED
	// Drop the oldest resource if we have reached our limit
	// it may no longer be needed.
	if (preloadResourceList_.size() == MAX_PRELOAD_COUNT)
	{		
		preloadResourceList_.front()->Release();
		preloadResourceList_.pop_front();
	}
