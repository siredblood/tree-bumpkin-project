...
/*virtual*/ LinkProxy::TargetState EntityLinkProxy::canLinkAtPos(ToolLocatorPtr locator_) const
{
	...
	if ( locator->chunkItem() != NULL && EditorChunkCache::instance( *entity_->chunk() ).edIsWriteable() )
		return TS_NO_TARGET;
...
/*virtual*/ void EntityLinkProxy::createLinkAtPos(ToolLocatorPtr locator_)
{
    ...
    for (size_t i = 0; i < entities.size(); ++i)
    {
        ...
        if( !EditorChunkCache::instance( *entity->chunk() ).edIsWriteable() )
	        continue;
    ...
    if (numLinked == 1)
        UndoRedo::instance().barrier(
			L("WORLDEDITOR/WORLDEDITOR/PROPERTIES/CHUNK_LINK_PROXY/LINK_ENTITY_TO_GRAPH"), false);
    else
        UndoRedo::instance().barrier(
			L("WORLDEDITOR/WORLDEDITOR/PROPERTIES/CHUNK_LINK_PROXY/LINK_ENTITIES_TO_GRAPH"), false);
}
