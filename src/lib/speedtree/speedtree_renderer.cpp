...
{
	return TSpeedTreeType::s_branchesEffect_;
}


static TSpeedTreeType::EffectPtr getLeavesEffect()
{
	return TSpeedTreeType::s_leavesEffect_;
}


static TSpeedTreeType::EffectPtr getBillboardsEffect()
{
	return TSpeedTreeType::s_billboardsEffect_;
}
// ---------------------------------------------------------- SpeedTreeSettings
...
public:
	...
	void refreshSettings() { this->setQualityOption( qualitySettings_->activeOption()); }
...
{
	CSpeedTreeRT::Authorize("35C7615F7F9D99F1");
...
{
	PROFILER_SCOPED( SpeedTreeRenderer_drawRenderPass );
...

	{
		...
		if ( !TSpeedTreeType::s_curShadowCaster_ )
		{
			...
			if ( standardLighting )
			{
				...
				{
					newLightingBatch = false;
				}
			}
			else
			{
				// see if there is a lighting container for this batch cookie.  It will
				// be the most recently added lighting container if there is one.
				if ( s_deferredLighting_.size() > 0 )
				{
					if ( s_deferredLighting_.back().batchCookie_ == batchCookie )
					{
						newLightingBatch = false;
					}
				}
	...
	bool commit )
{
	...
	// matrices instead of uploading each matrix at a time.
		...
		PROFILER_BEGIN( TSpeedTreeType_setEffectParam );
...
void TSpeedTreeType::drawTreePart( RendererVector& renderGroup, ETREE_PARTS part )
{
	...
	static Moo::EffectMaterialPtr effects[2][TPART_COUNT] =
	{
		{
	...
	renderGroup.back()->setCompositeRenderStates( effects[depthOnly][part] );
	partStateWatch.stop();
	...
	while ( renderIt != renderEnd )
	{
		...
		{
			(*renderIt)->uploadWindMatrixConstants( effects[depthOnly][part] );
...
void TSpeedTreeType::drawDeferredTrees()
{
	...
	FOREACH_TREETYPE_END
#endif
	static Moo::EffectMaterialPtr effects[TPART_COUNT] =
	{
		TSpeedTreeType::s_leavesEffect_.get(),
		TSpeedTreeType::s_branchesEffect_.get(),
		TSpeedTreeType::s_billboardsEffect_.get(),
...
{
	PROFILER_SCOPED( TSpeedTreeType_drawBranch );
...
{
	PROFILER_SCOPED( TSpeedTreeType_drawFrond );
...
void TSpeedTreeType::drawLeaf(
	...
	const DrawData     & drawData )
{
	PROFILER_SCOPED( TSpeedTreeType_drawLeaf );
...
{
	PROFILER_SCOPED( TSpeedTreeType_drawBillboard );
