	...
	resourceID_(resourceID)
{
	...
	while (rsit != rsend)
	{
		...
		while (geit != geend)
		{
			...
			if (isOK_ && bumped && !geometry.vertices_->bumpedFormat())
			{
	...
	if ( pFoundBSP && !bspRequiresMapping )
...
HRESULT Visual::draw( bool ignoreBoundingBox, bool useDefaultPose )
{
	...
	if (helper.shouldDraw(ignoreBoundingBox, bb_))
	{
		...
		helper.start( staticVertexColours_.valid(), bb_ );

		EffectVisualContext::instance().initConstants();
		EffectVisualContext::instance().staticLighting( staticVertexColours_.valid() );

		if (staticVertexColours_.valid())
			staticVertexColours_.set( 1, 0, sizeof( DWORD ) );
			...
			while (git != gend)
			{
				...
				while (pgit != pgend)
				{
					...
					if (!pMat->channel())
					{
						...
						{
							geometry.vertices_->setVertices(rc().mixedVertexProcessing(), staticVertexColours_.valid());
						}
		...
		if (staticVertexColours_.valid())
			VertexBuffer().set( 1, 0, sizeof( DWORD ) );
		helper.fini();
	}

	staticVertexColours_.release();
...
HRESULT Visual::batch( bool ignoreBoundingBox, bool useDefaultPose )
{
	...
	if (helper.shouldDraw(ignoreBoundingBox, bb_))
	{
		...
		helper.start( staticVertexColours_.valid(), bb_, false );
