 ...
 *	that the user must declare and implement the isClonable() and clone().
...
#define IMPLEMENT_ROOT_CONTENT( shortName, longName, width, height, icon ) \
   ...
   std::string getDisplayString() { return (longName); };\
   std::string getTabDisplayString() { return (shortName); };\
...
public:
	 ...
	 *  @return string containing the desired window title text.
	 */
	 ...
	 *  @return string containing the desired tab title short text.
	 */
	virtual std::string getTabDisplayString() = 0;
 ...
 *	{
 *		IMPLEMENT_BASIC_CONTENT( "A Test!", "A Test Content Panel Class!", 265, 430, NULL )
 *	public:
 *
