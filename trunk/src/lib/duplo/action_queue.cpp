...
void ActionQueue::flush()
{
	...
	{
		if (iter->action) iter->action->finish_ = -1.f;
	}
	actionState_.clear();
...
void ActionQueue::saveActionState( DataSectionPtr state )
{
	...
	// First save everything currently in the actionState_
	for ( uint32 i = 1; i < actionState_.size(); i++ )
	{
		AQElement &aqe = actionState_[i];
		
		DataSectionPtr actionState = state->newSection( "action" );
		actionState->writeString( "name", aqe.action->pSource_->name_ );
		actionState->writeFloat( "finish", aqe.action->finish_ );
		actionState->writeFloat( "passed", aqe.action->passed_ );
		actionState->writeFloat( "played", aqe.action->played_ );
		actionState->writeBool( "promoteMotion", aqe.promoteMotion );
		if ( aqe.link != NULL )
			actionState->writeString( "link", aqe.link->action->pSource_->name_ );
	}
...
bool ActionQueue::restoreActionState( DataSectionPtr state, SuperModel *pSuperModel, MatrixLiaison* pMatrix )
{
	...
	{
		DataSectionPtr savedAction = actions[i];
	...
	CueResponses * responses, bool promoteMotion, SuperModelActionPtr link )
{
	...
	if (link)
	{
		...
		for (uint i = 0; i < waiting_.size(); i++)
		{
			...
			while (look->link != NULL) { look = look->link; }
			if (look->action == link)
			{
				haqe = &waiting_[i];
				aqe = look;
		...
		if (aqe == NULL) for (uint i = 1; i < actionState_.size(); i++)
		{
			...
			if (look->action == link && head.action->passed_ <
				head.action->finish_ - head.action->pSource_->blendOutTime_ )
			{
				haqe = &head;
				aqe = look;
				break;
		...
		if (aqe != NULL)
		{
			aqe->link = new AQElement(
				action, responses, promoteMotion );
			haqe->action->finish_ = action->pSource_->filler_ ?
				-1.f : haqe->action->finish_ + action->pFirstAnim_->duration_;
			return;
		}
	}
...
void ActionQueue::stop( const ModelAction * pActionSource )
{
	...
	{
		AQElement & aqe = actionState_[i];
		if (aqe.action->pSource_ == pActionSource)
		{
			float newFinish = aqe.action->passed_ +
				aqe.action->pSource_->blendOutTime_;
			if (aqe.action->finish_ < 0.f || newFinish < aqe.action->finish_)
			{
				//DEBUG_MSG( "Setting finish on action '%s' to %g\n",
				//	aqe.action->actionName().c_str(), aqe.finish );
				aqe.action->finish_ = newFinish;
			}
		}
	}
	...
	for (uint i=0; i < waiting_.size(); i++)
	{
		...
		{
			aqe.doCallback( 0.f );
			for (AQElement * look = aqe.link; look != NULL; look = look->link)
			{
				look->doCallback( 0.f );
			}
...
void ActionQueue::getCurrentActions( std::vector<SuperModelActionPtr>& actionsInUse )
{
	...
	{
		AQElement & aqe = actionState_[i];
		actionsInUse.push_back( aqe.action );
	}
}
	...
	float actionTime, bool promoteMotion )
{
	...
	{
		AQElement	& aqe = actionState_[i];
		SuperModelActionPtr ia = aqe.action;
	...
	//TRACE_MSG( "ActionMatcher choose a different action: %s\n",
	//	pAction ? pAction->actionName().c_str() : "None" );

	float	cycleParam = 0.f;

	// ok, they're different, so start blending out the old one
	if (matchState.action)
	{
		// copy the cycle position of the old animation if it was movement
		if (matchState.action->pSource_->isMovement_ &&
			matchState.action->pFirstAnim_->looped_)
		{
			cycleParam = fmodf( matchState.action->played_ /
				matchState.action->pFirstAnim_->duration_, 1.0 );
		}

		matchState.action->finish_ = matchState.action->passed_ +
			matchState.action->pSource_->blendOutTime_ *
			matchState.action->blendRatio();
	...
	if (action)
	{
		...
		if (action->finish_ != -1)
		{
			...
			{
				if (actionState_[eraseIdx].action == action) break;
			}
		}

		matchActionTime_ = actionTime;

		newMatchState.action->played_ = playedTime;
		newMatchState.action->passed_ = passedTime;
		newMatchState.action->finish_ = -1.f;	// these are all like fillers
		newMatchState.action->lastPlayed_ = playedTime;
		newMatchState.responses = NULL;
		newMatchState.promoteMotion = promoteMotion;
		...
		if (eraseIdx > 0)
		{
			MF_ASSERT_DEV( eraseIdx < actionState_.size() &&
				actionState_[eraseIdx].responses == NULL );
...
{
	BW_GUARD;
	SuperModelActionPtr action = actionState_[0].action;
	if (action)
	{
...
void ActionQueue::tick( float dTime, SuperModel* supermodel, MatrixLiaison* pMatrix )
{
	...
	for (uint i = 0; i < waiting_.size(); i++)
	{
		...
		if (aqe.action->passed_ + dTime >= 0)
		{
			// pretend it's played for that long too
			aqe.action->played_ = aqe.action->passed_;

			// if we're not on track -1, first stop any
			//  actions already playing on the same track
			if (aqe.action->pSource_->track_ != -1)
			{
				int track = aqe.action->pSource_->track_;
				for (uint j = 1; j < actionState_.size(); j++)
				{
					AQElement & jaqe = actionState_[j];
					if (jaqe.action->pSource_->track_ == track)
					{
						float newFinish = jaqe.action->passed_ +
							jaqe.action->pSource_->blendOutTime_;
						float & oldFinish = jaqe.action->finish_;
			...
			fashionState_.push_back( aqe.action );
	...
	for (uint i = 1; i < actionState_.size(); i++)
	{
		...
		aqe.action->passed_ += dTime;
		aqe.action->played_ += dTime;

		// move on the links while they're being used up
		while (aqe.link != NULL && aqe.action->played_ >
			aqe.action->pFirstAnim_->duration_)
		{
			// set up link
			float odur = aqe.action->pFirstAnim_->duration_;
			aqe.link->action->passed_ = aqe.action->passed_;
			aqe.link->action->played_ = aqe.action->played_ - odur;
			aqe.link->action->finish_ = aqe.action->finish_;
			aqe.link->action->lastPlayed_ = 0.f;
			...
			if (aqe.responses != NULL)
			{
				CueResponse::respond( numCueDefaults, *aqe.responses, NULL );
			}
			...
			aqe.followLink();
			fashionState_[i] = aqe.action;
		}

		// see if we're getting rid of it now
		if (aqe.action->finish_ >= 0.f && aqe.action->finish_ < aqe.action->passed_)
		{
			//DEBUG_MSG( "Stopping action '%s', finish %g\n",
			//	aqe.action->pSource_->name_.c_str(), aqe.action->finish_ );

			float overShot = (aqe.action->passed_-aqe.action->finish_);
			this->endPlayingAction( aqe, dTime,
			//	aqe.action->pFirstAnim_->duration_ );
				aqe.action->played_ - overShot,
				supermodel, pMatrix );
	...
	if (actionState_[0].action)
	{
		AQElement	& matchState = actionState_[0];

		matchState.action->passed_ += dTime;

		// are we fiddling with the playback rate?
		if (matchActionTime_ >= 0.f)
		{
			//matchState.timeOffset -= dTime - matchActionTime_;
			matchState.action->played_ += matchActionTime_;
			matchActionTime_ = -1.f;
		}
		else
		{
			matchState.action->played_ += dTime;
		}
	}
	...
	for (uint i = 0; i < actionState_.size(); i++)
	{
		...
		if (!aqe.action || !aqe.promoteMotion) continue;

		this->tickPromotion( aqe, dTime, pMatrix );
	}

	// and call the internal tick on every action
	for (uint i = !actionState_[0].action; i < actionState_.size(); i++)
	{
		AQElement & aqe = actionState_[i];
		aqe.action->tick( *supermodel, dTime );
...
void ActionQueue::endPlayingAction( AQElement & aqe, float dTime, float endat, SuperModel* supermodel, MatrixLiaison* pMatrix )
{
	...
	float overShot = aqe.action->played_ - endat;

	// tick it internally up until the end
	float rplay = aqe.action->played_;
	aqe.action->played_ = endat;
	aqe.action->tick( *supermodel, dTime - overShot );
	aqe.action->played_ = rplay;
...
{
	BW_GUARD;
	SuperModelActionPtr action = aqe.action;
...
bool ActionQueue::mollifyWorldTransform( MatrixLiaison* pMatrix )
{
	...
	for (uint i=0; i < actionState_.size(); i++)
	{
		if (actionState_[i].promoteMotion) return false;
	}
...
bool ActionQueue::effect()
{
	...
	uint istart = actionState_[0].action ? 0 : 1;
...
{
	BW_GUARD;
	uint ibegin = actionState_[0].action ? 0 : 1;
...
void ActionQueue::debugTick( float dTime )
{
	...
	for (uint i = ibegin; i < iend; i++)
	{
		...
		{
			if (s_aqhist[j].smap_ == aqe.action &&
				s_aqhist[j].last_ == dts-1) break;
			if (s_aqhist[j].smap_->pSource_ == aqe.action->pSource_)
		...
		if (j == s_aqhist.size())
		{
			s_aqhist.push_back( AQEltHistory( aqe.action, ecidx ) );
		}
		s_aqhist[j].last_ = dts;
		s_aqhist[j].blends_.push_back( aqe.action->blendRatio() );
...
void ActionQueue::debugDraw()
{
	...
	Vector2	lineseg[3];
	Moo::Colour colour;
	static BwFont::FontPtr myfont = BwFont::FontManager::instance().get( "default_medium.font" );
	...
	for (uint i = 0; i < s_aqhist.size(); i++)
	{
		...
		BwFont::FontManager::instance().setMaterialActive( myfont );
...
{
	BW_GUARD;
	if (responses != NULL && !responses->front().cueBase_ &&
		responses->front().function_)
	{		// call the completion function ... later
		responses->front().respond( missedBy, NULL );
		responses->front().function_ = NULL;
	}
}
...
void ActionQueue::AQElement::followLink()
{
	...
	promoteMotion = link->promoteMotion;

	AQElement * olink = link;
	link = olink->link;
	olink->link = NULL;
...
/*~ function ActionQueuer.Get_displacement
 *
 *	This function gets the action's displacement on the pyModel as a Vector3.
 *
...
/*~ function ActionQueuer.Get_rotation
 *
 ...
 *	the coordinated action, it all synchronizes	properly.  Returns a vector4,
 *	with the 4th element being yaw.
 *
 *	@return Returns the action's rotation as a Vector4.
 */
/// Special get method for Python attribute 'rotation'
...
/*~ function ActionQueuer.Get_impact
 *
 *	This attribute returns what the final position of our
...
PY_BEGIN_ATTRIBUTES( ActionQueuer )
	 ...
	 *	accordingly, to keep it synchronized with the movement rate.  This
	 ...
	 *	the coordinated action, it all synchronizes	properly.  It is a vector4,
	 *	with the 4th element being yaw.
	 *
	 *	@type	Read-Only Vector4
	 */
