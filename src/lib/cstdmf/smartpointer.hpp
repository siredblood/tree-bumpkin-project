...
#define BS_NO_WATCHER_INCLUDE
#include "concurrency.hpp"
#undef BS_NO_WATCHER_INCLUDE
...
#if 0	// unused exceptions
struct proxy_error : public std::logic_error
{
	proxy_error( const std::string& W ) : std::logic_error( W ) { }
};

struct null_reference : public proxy_error
{
	null_reference( const std::string& ) : proxy_error( "null reference: " )
		{ }
};

struct bad_reference_count : public proxy_error
{
	bad_reference_count( int N ) : proxy_error( "bad ref count" ), count( N )
		{ }
	int count;
};

struct null_dereference : public proxy_error
{
	null_dereference( const std::string& ) : proxy_error("null dereference: ")
		{ }
};

struct null_const_dereference : public proxy_error
{
	null_const_dereference( const std::string& /*W*/ ) :
		proxy_error( "null_const_dereference: " ) { }
};

struct non_const_conversion : public proxy_error
{
	non_const_conversion( const std::string& /*W*/ ) :
		proxy_error( "non_const_conversion: " ) { }
};

struct dereference_error : public proxy_error
{
	dereference_error( const std::string& /*W*/ ) :
		proxy_error( "dereference_error: " ) { }
};

#endif	// unused exceptions
...
//		if (count_ <= 0)
//			throw bad_reference_count(count_-1);
		if (--count_ == 0)
