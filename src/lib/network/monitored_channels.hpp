 ...
 */
class MonitoredChannels : public TimerExpiryHandler
{
public:
	typedef std::list< Channel * > Channels;
	typedef Channels::iterator iterator;

	MonitoredChannels() : period_( 0.f ), timerID_( TIMER_ID_NONE ) {}

	void addIfNecessary( Channel & channel );
	void delIfNecessary( Channel & channel );
	void setPeriod( float seconds, Nub & nub );

	void stopMonitoring( Nub & nub );
