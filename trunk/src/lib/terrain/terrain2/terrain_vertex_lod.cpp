...
namespace
{
	...
	const float minNeighbourDistance = 
		sqrtf(BLOCK_SIZE_METERS*BLOCK_SIZE_METERS + 
			SUB_BLOCK_SIZE_METERS*SUB_BLOCK_SIZE_METERS);

	// The distance from a corner of the block to the centre of the block
	const float blockCentreDistance = 
		sqrtf(SUB_BLOCK_SIZE_METERS*SUB_BLOCK_SIZE_METERS + 
			SUB_BLOCK_SIZE_METERS*SUB_BLOCK_SIZE_METERS);

	const Vector3 blockNegativeX( -BLOCK_SIZE_METERS, 0, 0 );
	const Vector3 blockPositiveX( BLOCK_SIZE_METERS, 0, 0 );
	const Vector3 blockNegativeZ( 0, 0, -BLOCK_SIZE_METERS );
	const Vector3 blockPositiveZ( 0, 0, BLOCK_SIZE_METERS );

	const Vector3 subBlockOffset0( 0, 0, 0 );
	const Vector3 subBlockOffset1( SUB_BLOCK_SIZE_METERS, 0, 0 );
	const Vector3 subBlockOffset2( 0, 0, SUB_BLOCK_SIZE_METERS );
	const Vector3 subBlockOffset3( SUB_BLOCK_SIZE_METERS, 0, SUB_BLOCK_SIZE_METERS );
}
							...
							TerrainBlock2::LodRenderInfo& lodRenderInfo ) const
{
	...
	// Calculate neighbors
	const float blockSize = BLOCK_SIZE_METERS;
	...
	{	
		...
		const float subBlockSize = SUB_BLOCK_SIZE_METERS;
												...
												uint8			subBlockMask ) const
{
	...
	// non-diagonal neighbors have the same mask (ie the same lod). An entry
	// in the sub-block mask means we have the higher lod and must provide 
	// degenerates.
	
	// indexes
	uint8 myindex	= 0;	// my index		
	uint8 index1	= 1;	// first neighbor
	uint8 index2	= 2;	// second neighbor
	...
	// check neighbors and set, sub-block neighbor masks start at index 1.
	if ( myValue != value1 ) 
	{
		if ( myValue )
		{
...
namespace
{
	...
	struct PositionsAndMasks
	{
		...
		uint8	mask;		// direction from this to neighbor sub-block
		Vector3 sbOffset;	// world position of the neighbor sub-block.
	};
												...
												NeighbourMasks&	neighbourMasks ) const
{
	...
	static const float subBlockSize	= SUB_BLOCK_SIZE_METERS;
	...
	for ( uint32 i = 0; i < ARRAY_SIZE( testData ); i++ )
	{
		...
		// if we're a higher lod than our neighbor, then flag that direction for
		// degenerates
