...
public:
	...
	private:
		...
		BwFont::FontPtr font_;
		int last_;
		int lastGridLine_;
		double sps_;
		uint64 first_;		
		CustomMesh<Moo::VertexXYZDUV> timeLabelsMesh_;		
		CustomMesh<Moo::VertexXYZDUV> globalLabelsMesh_;
	...
	static void calculateDisplay( const DiaryEntries & entries, float height, XAxis& xAxis, uint64 now = timestamp(), int64 syncOffset = 0 );
	...
	static CustomMesh<Moo::VertexXYZDUV> textMesh_;
